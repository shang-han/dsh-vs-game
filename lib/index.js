...
export function toCharacter(g) {
  return {
    ...
    chests: (g.chests ?? []).map((c) => ({ id: c.id, kind: c.kind === 'record-player' ? 'record-player' : 'chest', x: Number(c.x) || 300, y: Number(c.y) || 340, slots: (c.slots ?? [null, null, null, null, null]).map((s2) => s2 && typeof s2 === 'object' && typeof s2.item === 'string' ? { item: s2.item, count: Math.max(1, Math.floor(Number(s2.count) || 1)) } : null) })),
...
export function sanitizeGlobal(g) {
  ...
  g.chests = g.chests.map((c, i) => ({
    ...
    kind: c?.kind === 'record-player' ? 'record-player' : 'chest',
...
export function openBagItem(g, item) {
  if (item !== 'newbie-gift' && item !== 'skill-book') return g;
  ...
  return { ...g, activeSkill: 'strike', inventory: inv, skillBookUsed: true };
}
...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      chests: z.array(z.object({ id: z.string(), kind: z.string().default('chest'), x: z.number().default(300), y: z.number().default(340), slots: z.array(z.union([z.object({ item: z.string(), count: z.number().default(1) }), z.null()])).max(5) })).default([]),
    ...
    switch (msg.kind) {
      ...
      case ClientMsg.CRAFT_ITEM: {
        ...
        mutateGlobal((g) => {
          ...
          const drop = { id: 'g' + Date.now().toString(36), item: product, x: 230 + (Math.random() * 40 - 20), y: 420 + (Math.random() * 40 - 20) };
      ...
      case ClientMsg.PLACE_CHEST: {
        ...
        const kind = msg.kind === 'record-player' ? 'record-player' : 'chest';
        const g0 = readGlobal();
        if (!(g0.inventory ?? []).includes(placeItem)) break;
        ...
        mutateGlobal((g) => {
          ...
          if (i >= 0) inv.splice(i, 1);
          const chests = [...(g.chests ?? []), { id: 'chest-' + Date.now().toString(36), kind, x, y, slots: [null, null, null, null, null] }];
          return { ...g, inventory: inv, chests };
        }).then((g) => sendCharacter(ws, g)).catch((e) => logger.warn('place container failed:', e));
        break;
      }
      case ClientMsg.EJECT_CONTAINER_ITEM: {
        const chestId = typeof msg.chestId === 'string' ? msg.chestId : null;
        const slot = Number(msg.slot);
        if (!chestId || !Number.isInteger(slot) || slot < 0 || slot > 4) break;
        mutateGlobal((g) => {
          ...
          const drop = { id: 'g' + Date.now().toString(36), item: st.item, x: Number(chest.x) || 300, y: (Number(chest.y) || 340) + 36 };
