...
const vsGameDomain = defineDomain({
  ...
  global: {
    ...
    initial: { bestScore: 0, totalKills: 0, totalRuns: 0, discovered: [] },
  },
});
...
function apply(ctx, config) {
  ...
  const readGlobal = () => ({
    bestScore: 0, totalKills: 0, totalRuns: 0, discovered: [],
  ...
  function handleClientMsg(ws, msg) {
    switch (msg.kind) {
      ...
      case ClientMsg.GAME_OVER: {
        ...
        saveRun(msg).then((g) => {
          ws.send(encodeMsg({ kind: HostMsg.SAVED, bestScore: g.bestScore, totalKills: g.totalKills, discovered: g.discovered }));
  ...
  async function saveRun(msg) {
    ...
    if (!persist.domain) {
      return { ...readGlobal(), bestScore: Math.max(readGlobal().bestScore, score) };
    }
    const g = readGlobal();
    const next = {
