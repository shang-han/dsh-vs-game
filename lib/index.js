...
function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (req.method === 'POST') {
        const jobId = 'job-' + (++updateJobSeq);
        const job = { id: jobId, status: 'running', output: '', profile: PROFILE_NAME, exitCode: null };
        ...
        child.on('close', (code) => {
          ...
          job.exitCode = code;
          setTimeout(() => updateJobs.delete(jobId), 60_000);
        });
        child.on('error', (e) => {
          job.status = 'failed';
        ...
        sendJson({ ok: true, jobId, status: 'running', profile: PROFILE_NAME });
        return;
      }
      res.writeHead(405);
      res.end();
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      if (!job) return sendJson({ ok: false, error: 'update job not found' }, 404);
      sendJson({ ok: true, id: job.id, status: job.status, output: job.output, profile: job.profile, exitCode: job.exitCode });
    },
  }), 'vs-game: update-job endpoint');
