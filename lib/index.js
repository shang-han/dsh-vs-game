...
function passiveUpgradeCost(level) {
  return 100 * (Number(level) + 1);
}

// ── P3 关底翻卡：奖池与发牌（服务端权威） ──
const FLIP_ACC_POOL = ['acc-ring', 'acc-boots', 'acc-shield', 'acc-grail', 'acc-lute', 'acc-flute',
  'acc-horn', 'acc-unicorn-horn', 'acc-conch', 'acc-uni-head', 'acc-medal-red', 'acc-medal-blue', 'acc-medal-green'];
const FLIP_MAT_POOL = ['mat-ingot-silver', 'mat-ingot-aqua', 'mat-ingot-blue', 'mat-ingot-purple', 'mat-ingot-rose', 'mat-ingot-green'];
function rollFlipCards() {
  const out = [];
  for (let i = 0; i < 3; i++) {
    const r = Math.random();
  ...
  function handleClientMsg(ws, msg) {
    switch (msg.kind) {
      ...
      case ClientMsg.BOSS_KILL: {
        ...
        ws._flip = { levelId, cards, picked: [], extraUsed: false };
        const respond = () => ws.send(encodeMsg({
          kind: HostMsg.CARDS, cards, freeFlips: 1, extraCost: 300,
      ...
      case ClientMsg.FLIP_PICK: {
        ...
        const allowance = f.freePicks ?? (1 + (f.extraUsed ? 1 : 0));
        if (f.picked.length >= allowance || f.picked.includes(i)) break;
        const card = f.cards[i];
        f.picked.push(i);
        mutateGlobal((g) => {
          if (card.kind === 'gold') return { ...g, gold: g.gold + Math.min(500, Math.max(0, Number(card.amount) || 0)) };
          if (typeof card.item === 'string' && /^(acc-|mat-|tool-)/.test(card.item)) return { ...g, inventory: [...g.inventory, card.item] };
          return g;
        })
      ...
      case ClientMsg.FLIP_EXTRA: {
        const f = ws._flip;
        if (!f || f.extraUsed) break;
        const gBefore = readGlobal().gold;
        if (gBefore < 300) break;
        mutateGlobal((g) => (g.gold >= 300 ? { ...g, gold: g.gold - 300 } : g))
          .then((g) => {
            if (g.gold >= gBefore) return; // 扣款失败（余额被并发消耗）
            f.extraUsed = true;
            ws.send(encodeMsg({ kind: HostMsg.CARD_RESULT, extraGranted: true, character: toCharacter(g) }));
          })
          .catch((e) => logger.warn('flip extra failed:', e));
        break;
      }
