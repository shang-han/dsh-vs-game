...
export const Config = Schema.object({
  ...
  windowScale: Schema.number().min(0.5).max(2).default(1),
});
...
function apply(ctx, config) {
  ...
  const cfg = { idleSpawnRate: 3, autoPause: true, difficulty: 'normal', windowScale: 1, ...config };
  // 异步就绪的持久化/设置句柄
  ...
  const publicConfig = () => ({
    ...
    windowScale: persist.settingsScope ? persist.settingsScope.get().windowScale : cfg.windowScale,
  });
  const snapshot = () => ({
