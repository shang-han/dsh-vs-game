...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      inventory: z.array(z.string()).default(['newbie-gift']),
      accessories: z.array(z.union([z.string(), z.null()])).default([null, null, null, null]),
      activeSkill: z.union([z.string(), z.null()]).default('strike'),
...
function apply(ctx, config) {
  ...
  const readGlobal = () => {
    ...
    // 清理早期演示用的药水/宝石，只保留新手礼包和技能书
    {
      ...
      // 主动技能：早期数据没有 activeSkill 时默认划除
      if (!g.activeSkill) g.activeSkill = 'strike';
    }
