...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      chests: z.array(z.object({ id: z.string(), slots: z.array(z.union([z.string(), z.null()])).max(5) })).default([]),
  ...
  function handleClientMsg(ws, msg) {
    switch (msg.kind) {
      ...
      case ClientMsg.SET_CRAFTING_STORAGE: {
        const items = Array.isArray(msg.items) ? msg.items.filter((x) => x === null || typeof x === 'string').slice(0, 11) : [];
        mutateGlobal((g) => ({ ...g, craftingStorage: items })).catch((e) => logger.warn('crafting storage failed:', e));
        break;
      }
      case ClientMsg.PLACE_CHEST: {
        ...
        mutateGlobal((g) => {
          ...
          if (i >= 0) inv.splice(i, 1);
          const chests = [...(g.chests ?? []), { id: 'chest-' + Date.now().toString(36), slots: [null, null, null, null, null] }];
