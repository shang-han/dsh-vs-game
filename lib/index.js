...
export function toCharacter(g) {
  return {
    ...
    chests: (g.chests ?? []).map((c) => ({ id: c.id, x: Number(c.x) || 300, y: Number(c.y) || 340, slots: c.slots ?? [null, null, null, null, null] })),
...
export function sanitizeGlobal(g) {
  ...
  g.chests = g.chests.map((c, i) => ({
    ...
    slots: Array.from({ length: 5 }, (_, j) => (typeof c?.slots?.[j] === 'string' ? c.slots[j] : null)),
  }));
  g.groundItems = g.groundItems.filter((x) => x && typeof x.id === 'string' && typeof x.item === 'string' && Number.isFinite(Number(x.x)) && Number.isFinite(Number(x.y)));
  ...
  if (!g.containerMigrated) {
    ...
    for (const c of g.chests) for (const item of c.slots) if (item) removeOne(item);
    g.inventory = inv;
...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      chests: z.array(z.object({ id: z.string(), x: z.number().default(300), y: z.number().default(340), slots: z.array(z.union([z.string(), z.null()])).max(5) })).default([]),
      craftingStorage: z.array(z.union([z.string(), z.null()])).max(11).default([]),
      groundItems: z.array(z.object({ id: z.string(), item: z.string(), x: z.number(), y: z.number() })).default([]),
  ...
  function handleClientMsg(ws, msg) {
    switch (msg.kind) {
      ...
      case ClientMsg.CRAFT_ITEM: {
        ...
        if (!product) break;
        const ingredients = Array.isArray(msg.ingredients) ? msg.ingredients : ['skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment','skill-fragment'];
        const need = new Map();
        for (const x of ingredients) need.set(x, (need.get(x) ?? 0) + 1);
        const g0 = readGlobal();
        if (![...need.entries()].every(([k, n]) => (g0.inventory ?? []).filter((x) => x === k).length >= n)) break;
        mutateGlobal((g) => {
          const remain = new Map(need);
          const inv = g.inventory.filter((x) => {
            const n = remain.get(x) ?? 0;
            if (n > 0) { remain.set(x, n - 1); return false; }
            return true;
          });
          return { ...g, inventory: [...inv, product] };
      ...
      case ClientMsg.SELL_ITEM: {
        ...
        const price = item.startsWith('acc-') ? 150 : item.startsWith('mat-') ? 80 : item === 'skill-fragment' ? 50 : item.startsWith('tool-') ? 120 : 10;
      ...
      case ClientMsg.PLACE_CHEST: {
        const g0 = readGlobal();
        if (!(g0.inventory ?? []).includes('wooden-chest')) break;
        const x = Math.max(30, Math.min(810, Number(msg.x) || 300));
        const y = Math.max(130, Math.min(490, Number(msg.y) || 340));
        mutateGlobal((g) => {
          const inv = [...g.inventory];
          const i = inv.indexOf('wooden-chest');
          if (i >= 0) inv.splice(i, 1);
          const chests = [...(g.chests ?? []), { id: 'chest-' + Date.now().toString(36), x, y, slots: [null, null, null, null, null] }];
          return { ...g, inventory: inv, chests };
        }).then((g) => sendCharacter(ws, g)).catch((e) => logger.warn('place chest failed:', e));
        break;
      }
      case ClientMsg.REMOVE_CHEST: {
        ...
        mutateGlobal((g) => {
          ...
          const addDrop = (item, i) => { drops.push({ id: 'g' + Date.now().toString(36) + i, item, x: baseX + (i % 3) * 24 - 24, y: baseY + Math.floor(i / 3) * 24 - 10 }); };
          let n = 0;
          for (const item of chest.slots ?? []) if (item) addDrop(item, n++);
          addDrop('mat-wood', n++);
      ...
      case ClientMsg.CHEST_TRANSFER: {
        ...
        mutateGlobal((g) => {
          ...
          if (!chest) return g;
          while (chest.slots.length < 5) chest.slots.push(null);
          const inv = [...g.inventory];
          if (msg.direction === 'in') {
            const item = typeof msg.item === 'string' ? msg.item : null;
            if (!item) return g;
            const i = inv.indexOf(item);
            const slot = chest.slots.findIndex((x) => !x);
            if (i < 0 || slot < 0) return g;
            inv.splice(i, 1);
            chest.slots[slot] = item;
          } else if (msg.direction === 'out') {
            const slot = Number(msg.slot);
            if (!Number.isInteger(slot) || slot < 0 || slot >= 5 || !chest.slots[slot]) return g;
