...
// ─── 游戏燃料状态（M3 的 reducer 会扩展这里） ──────────────────────────────
function createGameState(logger) {
  return {
    logger,
    lastActivity: 0,        // 最近一次真实工作事件时间戳（0 = 从未）
    totalTokens: 0,         // 累计吃到的 token（经验）
    waveDepth: 0,           // turn 嵌套深度
    /** 记录一次真实工作活动 */
    touch(now = Date.now()) {
      this.lastActivity = now;
    },
    isIdle(now = Date.now()) {
      return this.lastActivity === 0 || now - this.lastActivity > IDLE_THRESHOLD_MS;
    },
    snapshot() {
      return {
...
function apply(ctx, config) {
  ...
  const game = createGameState(logger);
  ...
  function onSessionEvent(session, event) {
    ...
    // 只有这些算"真实工作活动"（title 变更之类的不算）
    if (type === 'user/message' || type === 'assistant/message' || type === 'assistant/chunk'
      || type === 'tool/call' || type === 'tool/result' || type === 'turn/start' || type === 'turn/end') {
      game.touch();
    }
    // M3：在这里调用 game-reducer，把事件归约成燃料事件并 broadcast
  }

  // ── 5. 空闲保底刷怪定时器 ──
  ctx.effect(() => {
    const timer = setInterval(() => {
      if (!game.isIdle()) return;      // 有真实工作时让位
      if (wss.clients.size === 0) return; // 没有玩游戏的客户端就不刷
      broadcast({ kind: HostMsg.IDLE_SPAWN, difficulty: 1 });
    }, Math.max(1, cfg.idleSpawnRate) * 1000);
    return () => clearInterval(timer);
  }, 'vs-game: idle spawn timer');
