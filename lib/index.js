...
export function sanitizeGlobal(g) {
  ...
  // 测试期：保证角色身上至少有 10 个技能书碎片（后续可移除）
  const frags = g.inventory.filter((x) => x === 'skill-fragment').length;
  if (frags < 10) g.inventory = [...g.inventory, ...Array(10 - frags).fill('skill-fragment')];
  ...
  function handleClientMsg(ws, msg) {
    switch (msg.kind) {
      ...
      case ClientMsg.CRAFT_ITEM: {
        if (msg.item !== 'skill-book') break;
        const g0 = readGlobal();
        const frags = (g0.inventory ?? []).filter((x) => x === 'skill-fragment').length;
        if (frags < 10) break;
        mutateGlobal((g) => {
          let removed = 0;
          const inv = g.inventory.filter((x) => {
            if (x === 'skill-fragment' && removed < 10) { removed++; return false; }
            return true;
          });
          return { ...g, inventory: [...inv, 'skill-book'] };
