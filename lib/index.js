...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      inventory: z.array(z.string()).default(['newbie-gift', 'skill-book', 'potion-red', 'gem-ruby']),
...
function apply(ctx, config) {
  ...
  const readGlobal = () => {
    ...
    // 演示/迁移：只要新手礼包还没开过，就确保它一直在背包里
    if (!g.giftOpened) {
      for (const demo of ['newbie-gift', 'skill-book', 'potion-red', 'gem-ruby']) {
        if (!g.inventory.includes(demo)) g.inventory.push(demo);
      }
    }
    return g;
  };
