...
export function toCharacter(g) {
  return {
    ...
    chests: (g.chests ?? []).map((c) => ({ id: c.id, kind: c.kind === 'record-player' ? 'record-player' : 'chest', room: Number(c.room) || 0, x: Number(c.x) || 300, y: Number(c.y) || 340, slots: (c.slots ?? [null, null, null, null, null]).map((s2) => s2 && typeof s2 === 'object' && typeof s2.item === 'string' ? { item: s2.item, count: Math.max(1, Math.floor(Number(s2.count) || 1)) } : null) })),
...
const vsGameDomain = defineDomain({
  ...
  global: {
    schema: z.object({
      ...
      chests: z.array(z.object({ id: z.string(), kind: z.string().default('chest'), room: z.number().default(0), x: z.number().default(300), y: z.number().default(340), slots: z.array(z.union([z.object({ item: z.string(), count: z.number().default(1) }), z.null()])).max(5) })).default([]),
    ...
    switch (msg.kind) {
      ...
      case ClientMsg.EJECT_CONTAINER_ITEM: {
        ...
        mutateGlobal((g) => {
          ...
          const drop = { id: 'g' + Date.now().toString(36), item: st.item, room: Number(chest.room) || 0, x: Number(chest.x) || 300, y: (Number(chest.y) || 340) + 36 };
