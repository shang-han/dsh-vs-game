...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const MV_ITEM_NAMES = {
			...
			'mat-diamond': '钻石', 'record-player': '唱片机', 'record-billie-jean': 'Billie Jean 唱片', 'record-letmego': 'letmego 唱片',
		};
...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			placeContainerAt(x, y) {
				...
				this.sendWs({ kind: ClientMsg.PLACE_CHEST, placeItem: this.placingChestItem || 'wooden-chest', kind: this.placingChestKind || 'chest', x: px, y: py });
				this.placingChest = false;
			}
			...
			playRecord(item, chestId) {
				...
				const file = item === 'record-billie-jean' ? '/vs-game/assets/music/bgm/Billie Jean.mp3'
					: item === 'record-letmego' ? '/vs-game/assets/music/bgm/letmego.mp3' : null;
				if (!file) return;
			...
			finishCraft(product) {
				...
				c.placed = [];
				c.result = null;
				this.craftPops.push({ item: product, t: 0, dur: 0.5 });
				c.message = '合成成功';
				c.messageTimer = 1.6;
			}
			...
			tryStartCraft() {
				...
				// 目前唯一配方：10 个技能书碎片
				const valid = recipe.length === 10 && recipe.every((x) => x === 'skill-fragment');
				if (valid) {
					c.stage = 'crafting';
					c.timer = 2.2;
				} else {
			...
			// ── 主更新 ──
			tick(dt) {
				if (this.phase === 'home') { this.updateHome(dt); return; }
			...
			updatePlayer(dt) {
				...
				if (this.easterEgg && this.homeRoom === 1) { dx = -dx; dy = -dy; }
				p.moving = dx !== 0 || dy !== 0;
				if (p.moving) {
			...
			drawBrickRoom(c) {
				...
				const useInvert = this.easterEgg && this.homeRoom === 1 && this.recordPlaying
					? Math.floor((performance.now() - (this.recordPlaying.startAt || 0)) / 512) % 2 === 1 : false;
			...
			drawHomeObjects(c, t) {
				...
				for (const pop of (this.craftPops ?? [])) {
					...
					const px = 150 + prog * 80;
					const py = 420 - Math.sin(prog * Math.PI) * 70 + prog * 10;
		...
		function CraftModal({ character, placed, onChange, onStart, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				...
				/** 长按宝箱：打开 / 拆除 */
		function ChestActionModal({ onOpen, onRemove, onClose }) {
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 260, background: '#0e1017', border: '2px solid #8a6b3d', borderRadius: 14, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }, children: [
					h('div', { key: 't', style: { fontWeight: 700, textAlign: 'center' }, children: '📦 木制宝箱' }),
					h('button', { key: 'o', className: 'dsh-vs-btn', onClick: onOpen, children: '打开' }),
					h('button', { key: 'r', className: 'dsh-vs-btn ghost', onClick: onRemove, children: '拆除（返还 1 木材）' }),
				...
				/** 唱片机：左侧唱片，右侧一个播放槽 */
		function RecordPlayerModal({ character, index, send, onPlay, onClose }) {
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }, children: [
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 260, overflowY: 'auto' }, children: entries.map((r) => {
								...
								return h('div', { key: r.slot, className: 'dsh-vs-item', onClick: () => { if (container) send({ kind: ClientMsg.CHEST_TRANSFER, chestId: container.id, direction: 'in', item: r.item }); }, title: r.meta.name, children: [
									r.meta.iconUrl ? h('img', { key: 'i', className: 'dsh-vs-item-img', src: r.meta.iconUrl, alt: r.meta.name }) : h('div', { key: 'i', className: 'dsh-vs-item-icon', children: r.meta.icon }),
									r.count > 1 ? h('div', { key: 'c', className: 'dsh-vs-item-count', children: '×' + r.count }) : null,
									h('div', { key: 'n', className: 'dsh-vs-item-name', children: r.meta.name }),
								] });
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }, children: [
							...
							h('div', { key: 'slot', className: 'dsh-vs-item' + (slot ? ' use' : ' empty'), onClick: () => { if (slot && container) send({ kind: ClientMsg.CHEST_TRANSFER, chestId: container.id, direction: 'out', slot: 0 }); }, style: { width: 72, height: 72 }, children: playable ? [
							...
							h('button', { key: 'play', className: 'dsh-vs-btn', disabled: !playable, onClick: () => { if (playable && container) onPlay(slot.item, container.id); }, children: '播放' }),
		...
		function ShopModal({ character, send, onClose }) {
			...
			const buyPrice = (item) => item === 'mat-wood' ? 200
				...
				: 0;
			const buyRows = ['mat-wood', 'mat-ingot-silver', 'mat-diamond', 'record-billie-jean', 'record-letmego'].map((item) => ({ item, count: 99, price: buyPrice(item), meta: itemMeta(item), buy: true }));
					...
					nodes.push(h('div', {
						...
						children: [
							...
							canOpen && isSel ? h('button', {
								...
								onClick: (e) => {
									e.stopPropagation();
									if (onPlaceChest) onPlaceChest(item); else send({ kind: ClientMsg.PLACE_CHEST, placeItem: item, kind: item === 'record-player' ? 'record-player' : 'chest' });
									setSelectedItem(null);
								},
								children: '放置',
							}) : null,
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			const s = snap;
			return hs('div', {
				className: 'dsh-vs-win' + (standalone ? ' dsh-vs-standalone' : ''),
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase !== 'menu' && s.phase !== 'home' ? h(Hud, { key: 'hud', snap: s }) : null,
					...
					s && s.phase === 'bossintro' && s.bossIntro ? hs('div', {
						...
						children: [
							hs('div', { key: 'dialog', className: 'dsh-vs-dialog', children: [
								h('div', { key: 'sp', className: 'dsh-vs-dialog-speaker', children: s.bossIntro.lines[s.bossIntro.index]?.speaker ?? '' }),
								h('div', { key: 'tx', className: 'dsh-vs-dialog-text', children: s.bossIntro.lines[s.bossIntro.index]?.text ?? '' }),
				...
				recordOpen != null ? h(RecordPlayerModal, { key: 'record', character, index: recordOpen, send, onPlay: (item, chestId) => engineRef.current?.playRecord(item, chestId), onClose: () => { setRecordOpen(null); engineRef.current?.focusCanvas(); } }) : null,
				chestMenu != null ? h(ChestActionModal, { key: 'chestmenu',
					onOpen: () => { setChestOpen(chestMenu); setChestMenu(null); engineRef.current?.focusCanvas(); },
		...
		function VsGameRoot() {
			...
			return h(ItemTipProvider, { key: 'tipwrap', children: hs('div', { className: 'dsh-vs-root', children: [
				(open || standalone) ? h(GameWindow, { key: 'win', standalone, onClose: () => setOpen(false), wsStatus: status, send, helloRef, character, goldEarned, levels }) : null,
				!standalone ? h('button', {
					key: 'toggle',
