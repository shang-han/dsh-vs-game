...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			drawPlayer(c, t) {
				...
				const drawn = drawSprite(c, state, t, p.x, p.y, 56, flip);
				if (!drawn) {
					c.fillStyle = '#7c5cfc';
					c.beginPath();
		...
		function ChestActionModal({ onOpen, onRemove, onClose, title = '📦 木制宝箱', removeLabel = '拆除（返还 1 木材）' }) {
			return hs('div', { className: 'dsh-vs-cover', children: [
				...
				/** 唱片机：左侧唱片，右侧一个播放槽 */
		function RecordPlayerModal({ character, index, send, onPlay, onPause, onResume, onStop, onClose }) {
			...
			useEffect(() => { setStarted(false); setPaused(false); }, [slot?.item, container?.id]);
			const toggle = () => {
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				recordOpen != null ? h(RecordPlayerModal, { key: 'record', character, index: recordOpen, send, onPlay: (item, chestId) => engineRef.current?.playRecord(item, chestId), onPause: () => engineRef.current?.pauseRecord(), onResume: () => engineRef.current?.resumeRecord(), onStop: () => engineRef.current?.stopRecord(), onClose: () => { setRecordOpen(null); engineRef.current?.focusCanvas(); } }) : null,
