...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ClientMsg = {
			...
			CRAFT_ITEM: 'craft-item', SELL_ITEM: 'sell-item',
			EQUIP_ACCESSORY: 'equip-accessory', UNEQUIP_ACCESSORY: 'unequip-accessory',
			PLACE_CHEST: 'place-chest', REMOVE_CHEST: 'remove-chest', PICKUP_GROUND: 'pickup-ground', CHEST_TRANSFER: 'chest-transfer', SET_CRAFTING_STORAGE: 'set-crafting-storage',
		};
		...
		const ITEM_META = {
			...
			'skill-fragment': { icon: '📕', iconUrl: '/vs-game/assets/items/mv/mat-scroll.png', name: '技能书碎片·划除', desc: '收集 10 个可在合成台合成技能书' },
		};
		function itemMeta(item) {
			...
			if (MV_ITEM_NAMES[item]) {
				...
				if (item === 'acc-knife') {
					return { icon: '🔪', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], desc: '饰品', quality: 'white', attack: [5, 10] };
				}
				if (item === 'mat-wood') return { icon: '🪵', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], desc: '基础合成材料' };
				if (item === 'wooden-chest') return { icon: '📦', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], desc: '可放置的 5 格储物箱' };
				return {
					icon: isAcc ? '💍' : '🧱',
		...
		class GameEngine {
			...
			buildHomeItems() {
				const items = [
					...
					{ id: 'door', x: 750, y: 280, icon: '🚪', label: '出门' },
				];
				if (this.clearedLevels?.has('furious-user')) {
					items.push({ id: 'shop', x: 560, y: 430, icon: '🛒', label: '网上商店' });
				}
			...
			pickupGround(id) {
				this.sendWs({ kind: ClientMsg.PICKUP_GROUND, groundId: id.replace(/^ground:/, '') });
			}
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						if (this.placingChest) {
							...
							this.sendWs({ kind: ClientMsg.PLACE_CHEST, item: 'wooden-chest', x: px, y: py });
							this.placingChest = false;
							return;
						}
			...
			updateCrafting(dt) {
				...
				if (c.stage === 'crafting') {
					...
					if (c.timer <= 0) {
						...
						if (recipe) {
							c.result = recipe.product;
							c.stage = 'done';
							this.setBanner('✨ 合成完成，去拾取！');
						} else {
							c.stage = 'idle';
							c.message = '好像不太对';
							c.messageTimer = 1.6;
			...
			applyCharacter(c) {
				...
				this.chests = Array.isArray(c.chests) ? c.chests : [];
			...
			drawHomeItems(c, t) {
				...
				c.textBaseline = 'middle';
				for (const item of this.homeItems) {
					const bob = Math.sin(t * 2.2 + item.x) * 3;
		...
		function PediaModal({ snap, tab, setTab, onClose, character }) {
			...
			if (tab === 'weapon') {
				...
				body = CRAFT_RECIPES.map((r) => {
					const unlocked = Object.keys(r.need).some((m) => inv.has(m));
					const meta = itemMeta(r.product);
					const need = Object.entries(r.need).map(([item, n]) => itemMeta(item).name + ' ×' + n).join(' + ');
					return hs('div', { key: r.product, className: 'dsh-vs-pedia-card' + (unlocked ? '' : ' locked'), children: unlocked ? [
						...
						h('div', { key: 'ph', className: 'ph', children: [h('span', { key: 'i', children: '❓' }), h('span', { key: 'n', children: '未解锁配方' })] }),
						h('div', { key: 'd', className: 'pd', children: '获得该配方所需任意一种材料后解锁' }),
					] });
				});
			} else {
		...
		function CraftModal({ character, placed, onChange, onStart, onClose }) {
			...
			const leftRows = [...totals.entries()].map(([item, total]) => ({ item, total, placed: placedCounts.get(item) ?? 0, meta: itemMeta(item) })).filter((r) => r.total > 0);
			const add = (item) => {
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, minHeight: 300 }, children: [
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 280, overflowY: 'auto' }, children: leftRows.map((r) => {
								return h('div', { key: r.item, className: 'dsh-vs-item', onClick: () => add(r.item), title: r.meta.name + ' ×' + (r.total - r.placed), children: [
								...
								(r.total - r.placed) > 1 ? h('div', { key: 'c', className: 'dsh-vs-item-count', children: '×' + r.total }) : null,
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, minWidth: 0 }, children: [
							...
							hs('div', { key: 'ring', style: { position: 'relative', width: 190, height: 190, background: 'rgba(20,22,31,.4)', border: '1px dashed #7c5cfc', borderRadius: '50%', overflow: 'visible' }, children: [
								...Array.from({ length: 11 }, (_, i) => {
									const item = placed[i];
									return h('div', { key: i, onClick: () => { if (item) remove(i); }, title: item ? '点击取回' : '', style: ringStyle(i, 11), children: item ? (() => { const meta = itemMeta(item); return meta.iconUrl ? h('img', { key: 'ii', src: meta.iconUrl, style: { width: 26, height: 26, imageRendering: 'pixelated' } }) : h('span', { key: 'ic', style: { fontSize: 20 }, children: meta.icon }); })() : null });
								}),
		...
		function ChestModal({ character, index, send, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 660, minHeight: 420, background: '#0e1017', border: '2px solid #8a6b3d', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }, children: [
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }, children: [
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 260, overflowY: 'auto' }, children: [...counts.entries()].map(([item, count]) => {
								const meta = itemMeta(item);
								return h('div', { key: item, className: 'dsh-vs-item', onClick: () => { if (chest) send({ kind: ClientMsg.CHEST_TRANSFER, chestId: chest.id, direction: 'in', item }); }, title: meta.name, children: [
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							...
							hs('div', { key: 'slots', style: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }, children: slots.map((slot, i) => {
								const meta = slot ? itemMeta(slot) : null;
								return h('div', { key: i, className: 'dsh-vs-item' + (slot ? ' use' : ' empty'), onClick: () => { if (slot) send({ kind: ClientMsg.CHEST_TRANSFER, chestId: chest.id, direction: 'out', slot: i }); }, children: meta ? [
									meta.iconUrl ? h('img', { key: 'i', className: 'dsh-vs-item-img', src: meta.iconUrl, alt: meta.name }) : h('div', { key: 'i', className: 'dsh-vs-item-icon', children: meta.icon }),
									h('div', { key: 'n', className: 'dsh-vs-item-name', children: meta.name }),
								] : null });
							}) }),
				...
				/** 网上商店：购买/卖出标签页，左侧物品列表可滚动，右侧显示数量并确认交易 */
		function ShopModal({ character, send, onClose }) {
			const inventory = Array.isArray(character?.inventory) ? character.inventory : [];
			const counts = new Map();
			for (const it of inventory) counts.set(it, (counts.get(it) ?? 0) + 1);
			const price = (item) => item.startsWith('acc-') ? 150 : item.startsWith('mat-') ? 80 : item === 'skill-fragment' ? 50 : item.startsWith('tool-') ? 120 : 10;
			...
			const confirm = () => {
				if (!current || tab !== 'sell') return;
				const q = Math.max(1, Math.min(qty || 1, current.count));
				send({ kind: ClientMsg.SELL_ITEM, item: current.item, quantity: q });
				setSelected(null);
				setQty(1);
			};
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 620, minHeight: 460, backgroundImage: 'url("/vs-game/assets/ui/shop.png")', backgroundSize: '110% 110%', backgroundPosition: 'center', borderRadius: 18, padding: 34, display: 'flex', flexDirection: 'column', gap: 12, color: '#e6e8f0' }, children: [
					h('h2', { key: 'h', style: { marginLeft: '20ch', marginTop: '-2ch', textAlign: 'left' }, children: '🛒 商店' }),
					...
					hs('div', { key: 'body', style: { display: 'flex', gap: 14, minHeight: 250, alignItems: 'stretch' }, children: [
						hs('div', { key: 'list', style: { flex: '1 1 56%', overflowY: 'auto', maxHeight: 290, display: 'flex', flexDirection: 'column', gap: 6 }, children: rows.length === 0
							...
							: rows.map((r) => {
								...
								return hs('div', { key: r.item, onClick: () => pick(r), style: { display: 'flex', alignItems: 'center', gap: 10, background: isSel ? 'rgba(79,110,247,.22)' : 'rgba(10,12,18,.65)', border: '1px solid ' + (isSel ? '#4f6ef7' : '#2a2e3d'), borderRadius: 8, padding: '8px 12px', cursor: 'pointer' }, children: [
									...
									h('span', { key: 'p', className: 'sub', children: '+' + r.price + ' 金币' }),
								] });
							}) }),
						hs('div', { key: 'detail', style: { flex: '0 0 40%', background: 'rgba(10,12,18,.65)', border: '1px solid #2a2e3d', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }, children: current ? [
							...
							h('div', { key: 'dn', children: current.meta.name + ' ×' + current.count }),
							...
							h('button', { key: 'confirm', className: 'dsh-vs-btn', onClick: confirm, children: '确认卖出' }),
						] : [h('div', { key: 'tip', className: 'dsh-vs-empty', children: tab === 'sell' ? '点击左侧物品，右侧设置卖出数量' : '购买功能开发中' })] }),
					] }),
		...
		function CharacterModal({ character, goldEarned, send, onClose, onPlaceChest }) {
			...
			const renderBag = () => {
				...
				for (let i = 0; i < slotCount; i++) {
					...
					const canPlace = item === 'wooden-chest';
					...
					nodes.push(h('div', {
						...
						children: [
							...
							canOpen && isSel ? h('button', {
								...
								onClick: (e) => {
									e.stopPropagation();
									if (onPlaceChest) onPlaceChest(); else send({ kind: ClientMsg.PLACE_CHEST, item });
									setSelectedItem(null);
								},
								children: '放置',
							}) : null,
				...
				const selMeta = selectedItem ? itemMeta(selectedItem) : null;
				return hs('div', { children: [
					...
					selMeta ? hs('div', { key: 'detail', className: 'dsh-vs-item-detail', children: [
						h('b', { key: 'n', children: selMeta.name }),
						h('span', { key: 'd', className: 'sub', children: selMeta.desc }),
						selMeta.attack ? h('span', { key: 'a', style: { color: '#fff' }, children: '攻击 ' + selMeta.attack[0] + '-' + selMeta.attack[1] }) : null,
						selMeta.quality ? h('span', { key: 'q', style: { color: '#fff' }, children: '品质：白色' }) : null,
					] }) : null,
				] });
			};
							...
							hs('div', { key: 'pr', className: 'dsh-vs-char-portrait-row', children: [
								...
								hs('div', { key: 'acc', className: 'dsh-vs-char-acc-col', children: accessories.map((a, i) => {
									...
									return h('div', {
										...
										title: a ? (itemMeta(a).name + ' · ' + itemMeta(a).desc + '（点击卸下）') : '空饰品槽',
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			const handleHomeAction = (type) => {
				...
				else if (type.startsWith('chest:')) { setChestOpen(Number(type.split(':')[1])); }
			};
			...
			useEffect(() => {
				...
				const engine = new GameEngine(canvas, { sendWs: send, onHomeAction: handleHomeAction });
			...
			return hs('div', {
				...
				charOpen ? h(CharacterModal, { key: 'char', character, goldEarned, send, onClose: () => { setCharOpen(false); engineRef.current?.focusCanvas(); }, onPlaceChest: () => { setCharOpen(false); engineRef.current?.beginPlaceChest(); engineRef.current?.focusCanvas(); } }) : null,
			] });
		}
