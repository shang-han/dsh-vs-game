	...
	factory: (require) => {
		...
		function CraftModal({ character, placed, onChange, onClose }) {
			...
			const ringStyle = (i, total) => {
				...
				return { position: 'absolute', left: '50%', top: '50%', zIndex: 2, transform: 'translate(' + (Math.cos(angle) * 68) + 'px,' + (Math.sin(angle) * 68) + 'px) translate(-50%,-50%)', width: 42, height: 42, background: 'rgba(20,22,31,.85)', border: '1px solid #9d6bff', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' };
			};
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 720, minHeight: 460, background: '#0e1017', border: '2px solid #4f6ef7', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }, children: [
					...
					hs('div', { key: 'body', style: { display: 'flex', gap: 18, minHeight: 300 }, children: [
						hs('div', { key: 'left', style: { flex: '0 0 40%', display: 'flex', flexDirection: 'column', gap: 8 }, children: [
						...
						hs('div', { key: 'right', style: { flex: '1 1 auto', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }, children: [
							h('div', { key: 'rh', className: 'sub', children: '环形放置区 ' + placed.length + ' / 11' }),
							hs('div', { key: 'ring', style: { position: 'relative', width: 210, height: 210, background: 'rgba(20,22,31,.4)', border: '1px dashed #7c5cfc', borderRadius: '50%', overflow: 'visible' }, children: Array.from({ length: 11 }, (_, i) => {
								const item = placed[i];
