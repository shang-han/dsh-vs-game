...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ClientMsg = {
			...
			BUILD_PLACE: 'build-place', CHOP_TREE: 'chop-tree', BUILD_FINISH: 'build-finish',
		...
		const BUILD_ITEMS = {
			'tree-farm': { name: '树场', icon: '🌳', cost: { 'mat-wood': 10, 'mat-ingot-silver': 5 }, desc: '放在家里任意位置，按住 F 砍出木材', max: 1 },
		};
...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			attachInput() {
				...
				this._onKeyDown = (e) => {
					...
					if (k === 'tab' && this.phase === 'home') { this.toggleEditMode(); return; }
			...
			updateHome(dt) {
				...
				if (devNow && this.focused && this.keys.has('f') && this.buildCd <= 0) {
					...
					if (devNow.built === false) {
						...
						if (this.chopT >= 1.1 && this.chopCd <= 0) {
							this.chopT = 0;
							this.chopCd = 0.4;
							if (devNow.kind === 'tree-farm') {
								playSE('mine-place', 0.55, 0.05);
								this.burst(this.player.x, this.player.y - 16, '#8bc34a', 6);
								this.sendWs({ kind: ClientMsg.CHOP_TREE, deviceId: devNow.id });
							}
						}
					}
				} else {
			...
			drawHomeObjects(c, t) {
				...
				// 打造的家居器械：树场 = SF_Outside_B r1c14 + r2c14 竖拼两格
				for (let i = 0; i < (this.homeDevices ?? []).length; i++) {
					const dv = this.homeDevices[i];
					if ((Number(dv?.room) || 0) !== (this.homeRoom || 0)) continue;
					...
					if (dv.kind !== 'tree-farm') continue;
					const timg = tileImg('SF_Outside_B');
					if (!(timg.complete && timg.naturalWidth)) continue;
					const drawIt = () => {
						c.drawImage(timg, 14 * 48, 1 * 48, 48, 48, dx - 24, dy - 48, 48, 48);
						c.drawImage(timg, 14 * 48, 2 * 48, 48, 48, dx - 24, dy, 48, 48);
					};
					if (drag) c.globalAlpha = 0.6;
					if (dv.built !== false) { drawIt(); c.globalAlpha = 1; if (this.editMode) this.drawEditBox(c, dx, dy, 48, 96, !!(this.editSel && this.editSel.target === 'device' && this.editSel.index === i)); continue; }
					// 未建成：半透明虚影
					c.globalAlpha = drag ? 0.2 : 0.3;
					drawIt();
					c.globalAlpha = 1;
					// 施工中：从下往上变实
					if (this.editMode) this.drawEditBox(c, dx, dy, 48, 96, !!(this.editSel && this.editSel.target === 'device' && this.editSel.index === i));
					const prog = (this.buildTarget === dv.id) ? Math.min(1, (this.buildT ?? 0) / 3) : 0;
					if (prog > 0) {
						const bottom = dy + 48, full = 96;
						c.save();
						c.beginPath();
						c.rect(dx - 24, bottom - full * prog, 48, full * prog);
						c.clip();
						drawIt();
						c.restore();
					}
				...
				if (this.placingBuild && this.pointer) {
					...
					if (timg.complete && timg.naturalWidth) {
						c.drawImage(timg, 14 * 48, 1 * 48, 48, 48, this.pointer.x - 24, this.pointer.y - 48, 48, 48);
						c.drawImage(timg, 14 * 48, 2 * 48, 48, 48, this.pointer.x - 24, this.pointer.y, 48, 48);
					}
					c.globalAlpha = 1;
				}
				// 放置宝箱预览
			...
			drawHomeItems(c, t) {
				...
				// 打造器械进度环：施工(黄,3s) / 砍伐(绿,1.1s)
				if (this.homeNear && String(this.homeNear.id).startsWith('device:') && ((this.chopT ?? 0) > 0 || (this.buildT ?? 0) > 0)) {
					const item = this.homeNear;
					const high = item.y < 70;
					const fx = high ? item.x + 52 : item.x;
					const fy = high ? item.y + 10 : item.y - 44;
					const building = (this.buildT ?? 0) > 0;
					const prog = building ? Math.min(1, this.buildT / 3) : Math.min(1, this.chopT / 1.1);
					c.strokeStyle = building ? 'rgba(255,213,79,.95)' : 'rgba(139,195,74,.95)';
					c.lineWidth = 5;
					c.beginPath();
			...
			snapshot() {
				...
				const edit = (this.editMode && this.phase === 'home') ? { mode: true, sel: this.editSelInfo() } : null;
		...
		function ShopModal({ character, send, onClose, onBuild }) {
			...
			const buildRows = Object.entries(BUILD_ITEMS).map(([id, meta]) => {
		...
		function GameWindow({ standalone, hidden, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'home' ? h('button', { key: 'editToggle', className: 'dsh-vs-edit-toggle' + (s.edit && s.edit.mode ? ' on' : ''), onClick: () => engineRef.current?.toggleEditMode(), children: s.edit && s.edit.mode ? '✖ 退出编辑' : '✏️ 编辑模式' }) : null,
