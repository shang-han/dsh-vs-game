	...
	factory: (require) => {
		...
		class GameEngine {
			...
			updateCraftPops(dt) {
				...
				this.craftPops = this.craftPops.filter((p) => p.t < 1.1);
			}
			...
			finishCraft(product) {
				...
				c.placed = [];
				c.result = null;
				this.craftPops.push({ item: product, t: 0 });
				c.message = '合成成功';
				c.messageTimer = 1.6;
			}
			...
			drawHome(c, t) {
				...
				for (const pop of (this.craftPops ?? [])) {
					const prog = Math.min(1, pop.t / 1.1);
		...
		function CharacterModal({ character, goldEarned, send, onClose, onPlaceChest }) {
			...
			const renderBag = () => {
				...
				const counts = new Map();
				for (const it of inventory) counts.set(it, (counts.get(it) ?? 0) + 1);
				const order = bagOrderRef.current;
				for (const item of counts.keys()) if (!order.includes(item)) order.push(item);
				const entries = order.map((item) => ({ item, count: counts.get(item) ?? 0 }));
