	...
	factory: (require) => {
		...
		class GameEngine {
			...
			reset() {
				...
				this.shake = 0;
				this.slowField = null;   // 地雷 lv4 的减速场
				this.choices = null;
				this.best = null;
			}
			...
			updateEnemies(dt) {
				const p = this.player;
				const slowMul = (e) => (e.slow > 0 ? 0.5 : 1);
			...
			drawEnemies(c) {
				for (const e of this.enemies) {
					...
					if (e.elite && e.hp < e.maxHp) {
						c.fillStyle = '#1c1f2b';
						c.fillRect(e.x - 16, e.y - hh / 2 - 8, 32, 4);
						c.fillStyle = '#ff5f56';
						c.fillRect(e.x - 16, e.y - hh / 2 - 8, 32 * Math.max(0, e.hp / e.maxHp), 4);
					}
				}
			}
