...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-inv{display:grid;grid-template-columns:repeat(auto-fill,minmax(78px,1fr));gap:8px;align-content:start;overflow-y:auto;max-height:100%;}',
			'.dsh-vs-item{background:#0e1017;border:1px solid #2a2e3d;border-radius:10px;padding:8px 6px;display:flex;flex-direction:column;align-items:center;gap:3px;min-height:86px;box-sizing:border-box;}',
			'.dsh-vs-item.use{cursor:pointer;border-color:#5a4b2a;}',
			'.dsh-vs-item.use:hover{background:#1a1d29;border-color:#ffd54f;}',
			'.dsh-vs-item-icon{font-size:26px;line-height:1;}',
			'.dsh-vs-item-name{font-size:11px;font-weight:700;color:#e6e9f2;text-align:center;}',
			'.dsh-vs-item-desc{color:#8a8fa3;font-size:10px;text-align:center;line-height:1.3;}',
			'.dsh-vs-item-action{color:#ffd54f;font-size:10px;font-weight:700;margin-top:auto;}',
		...
		const ITEM_META = {
			'newbie-gift': { icon: '🎁', name: '新手礼包', desc: '内含 1000 金币' },
		};
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderBag = () => inventory.length > 0
				? hs('div', { className: 'dsh-vs-inv', children: inventory.map((item) => {
					...
					return hs('div', {
						...
						children: [
							...
							h('div', { key: 'd', className: 'dsh-vs-item-desc', children: canOpen ? meta.desc : '待开发' }),
							canOpen ? h('div', { key: 'a', className: 'dsh-vs-item-action', children: '点击打开' }) : null,
						],
					});
				}) })
				: h('div', { className: 'dsh-vs-empty', children: '背包空空如也' });
			...
			const renderPanel = () => hs('div', { className: 'dsh-vs-char-panel', children: [
				...
				h('div', { key: 'skill', className: 'dsh-vs-upgrade-row', children: [
					...
					h('span', { key: 'v', className: 'dsh-vs-upgrade-cost', children: char.activeSkill || '待开发' }),
				] }),
						...
						hs('div', { key: 'left', className: 'dsh-vs-char-left', children: [
							...
							hs('div', { key: 'pr', className: 'dsh-vs-char-portrait-row', children: [
								...
								hs('div', { key: 'acc', className: 'dsh-vs-char-acc-col', children: accessories.map((a, i) =>
									h('div', { key: i, className: 'dsh-vs-char-acc-slot', children: a || '未装备' })) }),
							] }),
							hs('div', { key: 'cards', className: 'dsh-vs-char-cards', children: [
								...
								hs('div', { key: 'skill', className: 'dsh-vs-char-card', children: [
									hs('div', { key: 'main', className: 'dsh-vs-char-card-main', children: [
										...
										hs('div', { key: 't', children: [
											h('div', { key: 'n', children: char.activeSkill || '主动技能' }),
											h('div', { key: 's', className: 'dsh-vs-char-card-sub', children: '待开发' }),
										] }),
									] }),
								] }),
							] }),
