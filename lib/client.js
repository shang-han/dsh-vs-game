...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function PediaModal({ snap, tab, setTab, onClose, character }) {
			...
			const craftUnlocked = CRAFT_RECIPES.some((r) => Object.keys(r.need).some((m) => inv.has(m)));
			...
			if (tab === 'weapon') {
				...
				body = craftUnlocked ? CRAFT_RECIPES.map((r) => {
				...
				}) : [h('div', { key: 'lock', className: 'dsh-vs-empty', children: '获得任意一种合成材料后解锁全部配方' })];
			} else {
