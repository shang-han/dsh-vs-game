...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-inv{display:flex;flex-direction:column;gap:6px;}',
			'.dsh-vs-item{background:#0e1017;border:1px solid #2a2e3d;border-radius:8px;padding:8px 10px;display:flex;align-items:center;justify-content:space-between;gap:8px;}',
			'.dsh-vs-item-name{font-size:12px;font-weight:700;}',
			'.dsh-vs-item-desc{color:#8a8fa3;font-size:11px;flex:1;}',
			...
			'.dsh-vs-char-portrait-canvas{width:220px;height:220px;background:#0e1017;border:1px solid #2a2e3d;border-radius:14px;}',
			'.dsh-vs-char-acc-col{display:flex;flex-direction:column;gap:6px;}',
			'.dsh-vs-char-acc-slot{width:84px;height:60px;background:#0e1017;border:1px dashed #2a2e3d;border-radius:10px;display:flex;align-items:center;justify-content:center;color:#5a6072;font-size:11px;}',
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderBag = () => inventory.length > 0
				? hs('div', { className: 'dsh-vs-inv', children: inventory.map((item) => {
					if (item === 'newbie-gift') {
						return hs('div', { key: item, className: 'dsh-vs-item', children: [
							h('div', { key: 'n', className: 'dsh-vs-item-name', children: '🎁 新手礼包' }),
							h('div', { key: 'd', className: 'dsh-vs-item-desc', children: '内含 1000 金币' }),
							h('button', { key: 'b', className: 'dsh-vs-mini-btn', onClick: () => send({ kind: ClientMsg.OPEN_ITEM, item }), children: '打开' }),
						] });
					}
					return hs('div', { key: item, className: 'dsh-vs-item', children: [
						h('div', { key: 'n', className: 'dsh-vs-item-name', children: item }),
						h('div', { key: 'd', className: 'dsh-vs-item-desc', children: '待开发' }),
					] });
				}) })
					...
					hs('div', { key: 'body', className: 'dsh-vs-char-body', children: [
						hs('div', { key: 'left', className: 'dsh-vs-char-left', children: [
							...
							hs('div', { key: 'pr', className: 'dsh-vs-char-portrait-row', children: [
								...
								hs('div', { key: 'acc', className: 'dsh-vs-char-acc-col', children: [
									h('div', { key: 't', className: 'dsh-vs-char-section-title', children: '饰品栏' }),
									accessories.map((a, i) =>
