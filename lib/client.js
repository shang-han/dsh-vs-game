		...
		class GameEngine {
			...
			tryDash(tx, ty) {
				...
				if (!this.player.skillAction) {
					this.player.skillAction = { t: 0, dur: 0.45, dx: (tx - x1) / dLen, dy: (ty - y1) / dLen };
			...
			updatePlayer(dt) {
				...
				if (p.moving) {
					...
					p.y += (dy / len) * spd * dt;
					if (dx !== 0) p.facing = dx > 0 ? 1 : -1;
					// 划除：移动时播放完整刺击动作帧；已有动作时不打断
					if (this.skillTimer > 0 && p.actionCd <= 0 && !p.skillAction) {
						p.skillAction = { t: 0, dur: 0.45, dx: dx / len, dy: dy / len };
						p.actionCd = 0.6;
					}
				}
				p.x = Math.max(16, Math.min(this.world.w - 16, p.x));
