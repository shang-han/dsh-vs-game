...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-item-img{width:28px;height:28px;image-rendering:pixelated;object-fit:contain;}',
			...
			'.dsh-vs-inv-wrap{display:flex;gap:10px;align-items:flex-start;min-height:0;}',
			'.dsh-vs-inv{flex:1;min-width:0;}',
			'.dsh-vs-item.selected{border-color:#ffd54f;background:#171a26;box-shadow:0 0 10px rgba(255,213,79,.25);}',
			'.dsh-vs-inv-side{width:150px;flex:none;background:#0e1017;border:1px solid #2a2e3d;border-radius:10px;padding:12px 10px;display:flex;flex-direction:column;align-items:center;gap:8px;text-align:center;}',
			'.dsh-vs-inv-side img{width:64px;height:64px;image-rendering:pixelated;object-fit:contain;}',
			'.dsh-vs-inv-side-icon{font-size:48px;line-height:1;}',
			'.dsh-vs-inv-side-name{font-size:13px;font-weight:700;color:#e6e9f2;}',
			'.dsh-vs-inv-side-desc{color:#8a8fa3;font-size:11px;line-height:1.4;}',
		...
		function itemMeta(item) {
			return ITEM_META[item] || { icon: '📦', name: item, desc: '待开发' };
		}
		/** 超武进化线：武器满级 + 指定被动 → 进化 */
		const EVOLUTIONS = {
		...
		class GameEngine {
			...
			attachInput() {
				...
				this._onKeyDown = (e) => {
					...
					if (['w', 'a', 's', 'd', 'arrowup', 'arrowdown', 'arrowleft', 'arrowright', ' ', '1', '2', '3', 'e', 'p', 'escape'].includes(k)) {
						e.preventDefault();
						e.stopPropagation();
					}
				...
				this._onPointerDown = (e) => {
					...
					this.tryDash(e.clientX - rect.left, e.clientY - rect.top);
				};
			...
			tryDash(tx, ty) {
				...
				tx = Math.max(14, Math.min(GAME_W - 14, tx));
				ty = Math.max(14, Math.min(GAME_H - 14, ty));
			...
			updateEnemies(dt) {
				...
				for (const e of this.enemies) {
					...
					if (e.elite && !e.boss) {
						// 精英怪远程输出：入图后不追人，随机游走
						const inside = e.x > 16 && e.x < GAME_W - 16 && e.y > 16 && e.y < GAME_H - 16;
						if (!inside) {
							dx = GAME_W / 2 - e.x;
							dy = GAME_H / 2 - e.y;
							...
							if (e.x > GAME_W - 16) { e.x = GAME_W - 16; e.wanderA = Math.PI + rand(-Math.PI / 2, Math.PI / 2); }
							if (e.y < 16) { e.y = 16; e.wanderA = rand(0, Math.PI); }
							if (e.y > GAME_H - 16) { e.y = GAME_H - 16; e.wanderA = Math.PI + rand(0, Math.PI); }
						}
					} else {
						e.x += (dx / d) * spd * dt;
						e.y += (dy / d) * spd * dt;
			...
			findBoltTarget(x, y, preferHp = false) {
				const inRange = this.enemies.filter((e) => Math.hypot(e.x - x, e.y - y) < 800);
			...
			fireBolt(lv) {
				...
				if (evo) {
					...
					for (let i = 0; i < 12; i++) {
						setTimeout(() => {
							if (this.phase !== 'playing' && this.phase !== 'dying') return;
							const t = this.findBoltTarget(this.player.x, this.player.y, true);
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderBag = () => {
				const INV_COLS = 6;
				const INV_SLOTS = 24;
				const slotCount = Math.max(INV_SLOTS, Math.ceil(inventory.length / INV_COLS) * INV_COLS);
				const nodes = [];
				for (let i = 0; i < slotCount; i++) {
					const item = inventory[i];
					if (!item) {
						nodes.push(h('div', { key: 'e' + i, className: 'dsh-vs-item empty' }));
						continue;
					}
					...
					nodes.push(h('div', {
						key: item,
						className: 'dsh-vs-item' + (canOpen ? ' use' : '') + (selectedItem === item ? ' selected' : ''),
						...
						children: [
							...
							canOpen ? h('div', { key: 'a', className: 'dsh-vs-item-action', children: item === 'skill-book' ? '用' : '开' }) : null,
						],
					}));
				}
				const sel = selectedItem ? itemMeta(selectedItem) : null;
				const selCanOpen = selectedItem === 'newbie-gift' || selectedItem === 'skill-book';
				return hs('div', { className: 'dsh-vs-inv-wrap', children: [
					hs('div', { className: 'dsh-vs-inv', children: nodes }),
					hs('div', { className: 'dsh-vs-inv-side', children: sel ? [
						sel.iconUrl
							? h('img', { key: 'i', src: sel.iconUrl, alt: sel.name })
							: h('div', { key: 'i', className: 'dsh-vs-inv-side-icon', children: sel.icon }),
						h('div', { key: 'n', className: 'dsh-vs-inv-side-name', children: sel.name }),
						h('div', { key: 'd', className: 'dsh-vs-inv-side-desc', children: sel.desc }),
						selCanOpen
							? h('button', {
								key: 'u',
								className: 'dsh-vs-mini-btn',
								onClick: () => send({ kind: ClientMsg.OPEN_ITEM, item: selectedItem }),
								children: selectedItem === 'newbie-gift' ? '打开' : '使用',
							})
							: h('div', { key: 'u', style: { color: '#5a6072', fontSize: 11 }, children: '暂不可用' }),
					] : [
						h('div', { key: 'tip', style: { color: '#5a6072', fontSize: 12 }, children: '选择物品查看操作' }),
					] }),
				] });
			};
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned }) {
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					!s || s.phase === 'menu' ? hs('div', { key: 'menu', className: 'dsh-vs-cover', children: [
						...
						h('button', { key: 'go', className: 'dsh-vs-btn', onClick: () => startGame(), children: '开始游戏' }),
						h('button', {
							key: 'dex',
							className: 'dsh-vs-btn ghost',
					...
					s && s.phase === 'gameover' ? hs('div', { key: 'over', className: 'dsh-vs-cover', children: [
						...
						h('button', { key: 'again', className: 'dsh-vs-btn', onClick: () => startGame(), children: '再来一局' }),
		...
		function VsGameRoot() {
			...
			return hs('div', { className: 'dsh-vs-root', children: [
				open ? h(GameWindow, { key: 'win', onClose: () => setOpen(false), wsStatus: status, send, helloRef, character, goldEarned }) : null,
				h('button', {
					key: 'toggle',
