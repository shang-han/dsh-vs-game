	...
	factory: (require) => {
		...
		class GameEngine {
			...
			enterHome() {
				...
				this.crafting = { stage: 'idle', timer: 0, recipe: [], placed: [], message: null, messageTimer: 0, x: 150, y: 420 };
			...
			updateHome(dt) {
				...
				if (this.homeNear && this.focused && this.keys.has('f') && this.homeFCooldown <= 0) {
					...
					if (this.homeNear.id === 'craft') {
						if (this.crafting?.stage === 'done') this.handleCraftInteraction();
						else this.onHomeAction('craft');
					} else this.onHomeAction(this.homeNear.id);
				}
			}
			...
			handleCraftInteraction() {
				...
				if (c.stage === 'idle') {
					...
					this.sendWs({ kind: ClientMsg.CRAFT_ITEM, item: 'skill-book' });
					c.stage = 'idle';
					c.recipe = [];
					c.placed = [];
					this.setBanner('🎒 技能书·划除已放入背包');
				}
			}
			...
			updateCrafting(dt) {
				...
				if (c.stage === 'crafting') {
					c.timer -= dt;
					if (c.timer <= 0) {
						const valid = c.recipe.length === 10 && c.recipe.every((x) => x === 'skill-fragment');
						if (valid) {
							c.stage = 'done';
			...
			drawHome(c, t) {
				...
				if (dungeon.complete && dungeon.naturalWidth) {
					...
					// 合成中：碎片飞入 / 完成后成品
					if (this.crafting?.stage === 'crafting') {
						const fragImg = itemImg('mat-scroll');
						if (fragImg.complete && fragImg.naturalWidth) {
							for (let i = 0; i < 10; i++) {
								const a = (i / 10) * Math.PI * 2 + t * 3;
								const px = 150 + Math.cos(a) * 26;
								const py = 420 + Math.sin(a) * 26;
								c.drawImage(fragImg, px - 9, py - 9, 18, 18);
							}
						}
					} else if (this.crafting?.stage === 'done') {
						const bookImg = itemImg('skill-book');
				...
				if (this.crafting?.message) {
					const p = this.player;
					c.fillStyle = 'rgba(20,22,31,.92)';
					c.fillRect(p.x - 42, p.y - 70, 84, 26);
					c.fillStyle = '#fff';
					c.font = '13px system-ui,sans-serif';
					c.textAlign = 'center';
					c.fillText(this.crafting.message, p.x, p.y - 52);
				}
				c.restore();
			}
		...
		function CraftModal({ character, placed, onChange, onClose }) {
			...
			const ringStyle = (i, total) => {
				...
				return { position: 'absolute', left: '50%', top: '50%', transform: 'translate(' + (Math.cos(angle) * 78) + 'px,' + (Math.sin(angle) * 78) + 'px) translate(-50%,-50%)', width: 48, height: 48, background: 'rgba(20,22,31,.85)', border: '1px solid #9d6bff', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' };
			};
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 660, minHeight: 460, background: '#0e1017', border: '2px solid #4f6ef7', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }, children: [
					h('h2', { key: 'h', children: '⚒️ 合成仪式' }),
					hs('div', { key: 'body', style: { display: 'flex', gap: 18, minHeight: 300 }, children: [
						hs('div', { key: 'left', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', gap: 8 }, children: [
							h('div', { key: 'lh', className: 'sub', children: '背包（点击放入）' }),
							hs('div', { key: 'grid', style: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, overflowY: 'auto', maxHeight: 280 }, children: leftRows.map((r) => {
								return h('div', { key: r.item, onClick: () => add(r.item), style: { aspectRatio: 1, background: '#1c1f2b', border: '1px solid #2a2e3d', borderRadius: 8, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 2, cursor: 'pointer' }, children: [
									h('span', { key: 'i', children: r.meta.iconUrl ? h('img', { key: 'ii', src: r.meta.iconUrl, style: { width: 28, height: 28, imageRendering: 'pixelated' } }) : r.meta.icon }),
									h('span', { key: 'n', className: 'sub', children: r.meta.name }),
									h('span', { key: 'c', className: 'sub', children: '×' + (r.total - r.placed) }),
								] });
							}) }),
						] }),
						hs('div', { key: 'right', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }, children: [
							h('div', { key: 'rh', className: 'sub', children: '环形放置区 ' + placed.length + ' / 11' }),
							hs('div', { key: 'ring', style: { position: 'relative', width: 220, height: 220, background: 'rgba(20,22,31,.4)', border: '1px dashed #7c5cfc', borderRadius: '50%' }, children: Array.from({ length: 11 }, (_, i) => {
								const item = placed[i];
								return h('div', { key: i, onClick: () => { if (item) remove(i); }, title: item ? '点击取回' : '', style: ringStyle(i, 11), children: item ? (() => { const meta = itemMeta(item); return meta.iconUrl ? h('img', { key: 'ii', src: meta.iconUrl, style: { width: 28, height: 28, imageRendering: 'pixelated' } }) : h('span', { key: 'ic', style: { fontSize: 22 }, children: meta.icon }); })() : null });
							}) }),
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				pediaOpen ? h(PediaModal, { key: 'pedia', snap: s, tab: pediaTab, setTab: setPediaTab, onClose: () => { setPediaOpen(false); engineRef.current?.focusCanvas(); } }) : null,
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, placed: craftPlaced, onChange: (items) => { setCraftPlaced(items); engineRef.current?.setCraftPlaced(items); }, onClose: () => { setCraftOpen(false); engineRef.current?.focusCanvas(); } }) : null,
