	...
	factory: (require) => {
		...
		class GameEngine {
			...
			enterHome() {
				...
				this.crafting = { stage: 'idle', timer: 0, x: 150, y: 420 };
			...
			handleCraftInteraction() {
				...
				if (!c) return;
				const frags = this.countFragments();
				if (c.stage === 'idle' && frags >= 10) {
					c.stage = 'crafting';
					c.timer = 2.2;
					this.setBanner('⚒️ 碎片放入法阵，开始合成……');
			...
			drawHome(c, t) {
				...
				if (dungeon.complete && dungeon.naturalWidth) {
					...
					// 合成中：右边蓝色发光 3x3（r10-12 c3-5）
					if (this.crafting?.stage === 'crafting') {
						for (let row = 10; row <= 12; row++) {
							for (let col = 3; col <= 5; col++) {
								c.drawImage(dungeon, col * 48, row * 48, 48, 48, 222 + (col - 3) * 48, 348 + (row - 10) * 48, 48, 48);
							}
						}
					}
					// 合成中：碎片飞入 / 完成后成品
