		...
		function Hud({ snap, activeLevel, character }) {
			return hs('div', { className: 'dsh-vs-hud', children: [
				...
				activeLevel?.id === 'busy-server' && (character?.clearedLevels ?? []).includes('busy-server') && snap.phase === 'playing'
					? h('button', { key: 'skipBoss', className: 'dsh-vs-btn ghost', style: { position: 'absolute', right: 10, top: 76, pointerEvents: 'auto', zIndex: 9, padding: '4px 10px', fontSize: 12 }, onClick: () => engineRef.current?.skipToBoss(), children: '⏩ 直接 Boss' })
					: null,
