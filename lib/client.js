...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 超武进化线：武器满级 + 指定被动 → 进化 */
		const EVOLUTIONS = {
			whip:  { passive: 'might',  name: '鲸尾横扫',   icon: '🐋', desc: '四道巨型鲸环扫荡 + 击退' },
			bolt:  { passive: 'haste',  name: '流式输出',   icon: '🌊', desc: '三连发高速弹，穿透 3' },
			orb:   { passive: 'magnet', name: '上下文窗口', icon: '🪟', desc: '6 球大半径，伤害翻倍并吸附宝石' },
			laser: { passive: 'armor',  name: '全量类型检查', icon: '🔍', desc: '12 道旋转激光网' },
			mine:  { passive: 'regen',  name: '垃圾回收',   icon: '♻️', desc: '存 6 雷，伤害翻倍，爆炸全减速' },
			zap:   { passive: 'speed',  name: '热重载',     icon: '🔥', desc: '4 道连锁闪电，冷却减半' },
		};
		/** 敌人图鉴中文名 */
		const ENEMY_NAMES = {
		...
		class GameEngine {
			...
			updateWeapons(dt) {
				...
				if (orbLv > 0) {
					...
					const orbR = orbEvo ? 95 : orbLv >= 3 ? 78 : 62;
					const count = orbEvo ? 6 : orbLv >= 4 ? 4 : orbLv >= 2 ? 3 : 2;
					const dmg = (orbEvo ? 4 : orbLv >= 4 ? 2.0 : 1.2) * this.dmgMul();
					if (orbEvo) {
			...
			fireBolt(lv) {
				...
				const p = this.player;
				const shots = evo ? 3 : lv >= 2 ? 2 : 1;
				const pierce = evo ? 3 : lv >= 4 ? 2 : lv >= 3 ? 1 : 0;
				const speed = evo ? 640 : lv >= 4 ? 540 : 420;
				const dmg = (evo ? 4.5 : lv >= 3 ? 3.5 : 2.5) * this.dmgMul();
			...
			fireLaser(lv) {
				...
				const dmg = (evo ? 7 : lv >= 3 ? 5 : 3) * this.dmgMul();
				const dur = evo ? 0.6 : lv >= 4 ? 0.5 : 0.28;
				const spin = evo ? this.elapsed * 0.8 : 0;
				for (let i = 0; i < n; i++) {
					const a = (i * Math.PI * 2) / n + (lv >= 2 ? Math.PI / 8 : 0) + spin;
					this.beams.push({ x: this.player.x, y: this.player.y, angle: a, len: evo ? 340 : 280, width: evo ? 14 : 12, damage: dmg, life: dur, maxLife: dur, hitSet: new Set() });
				}
			}
			...
			fireMine(lv) {
				...
				this.mines.push({
					x: this.player.x, y: this.player.y,
					radius: evo ? 95 : lv >= 3 ? 78 : 60,
					damage: (evo ? 20 : lv >= 3 ? 10 : 7) * this.dmgMul(),
			...
			fireZap(lv) {
				...
				const strikes = evo ? 4 : lv >= 2 ? 2 : 1;
				const dmg = (evo ? 8 : lv >= 4 ? 6 : 4) * this.dmgMul();
				const radius = evo ? 75 : lv >= 4 ? 62 : 46;
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					!s || s.phase === 'menu' ? hs('div', { key: 'menu', className: 'dsh-vs-cover', children: [
						...
						s && s.discovered && s.discovered.length > 0 ? hs('div', { key: 'dex', style: { maxWidth: 560, textAlign: 'center' }, children: [
							h('div', { key: 't', style: { color: '#8a8fa3', fontSize: 12, marginBottom: 6 }, children: '📖 敌人图鉴（' + s.discovered.length + '/' + Object.keys(ENEMY_TYPES).length + '）' }),
							h('div', { key: 'l', style: { display: 'flex', flexWrap: 'wrap', gap: 6, justifyContent: 'center' }, children: s.discovered.map((t) =>
								h('span', {
									key: t,
									style: { fontSize: 11, padding: '2px 8px', borderRadius: 6, border: '1px solid #2a2e3d', color: ENEMY_TYPES[t]?.color ?? '#ccc', background: 'rgba(20,22,31,.8)' },
									children: (ENEMY_TYPES[t]?.label ?? '??') + ' ' + (ENEMY_NAMES[t] ?? t),
								})),
							}),
						] }) : null,
