		...
		class GameEngine {
			...
			drawHome(c, t) {
				...
				for (let y = 0; y < 96; y += 48) {
					c.fillRect(0, y, this.world.w, 2);
				}
				// 木板地面
				c.fillStyle = '#6d5540';
				c.fillRect(0, 96, this.world.w, this.world.h - 96);
				c.strokeStyle = 'rgba(0,0,0,.18)';
				c.lineWidth = 1;
				for (let y = 96; y < this.world.h; y += 32) {
					c.beginPath();
					c.moveTo(0, y);
					c.lineTo(this.world.w, y);
					c.stroke();
				}
				for (let y = 96; y < this.world.h; y += 32) {
					for (let x = ((y / 32) % 2) * 96; x < this.world.w; x += 192) {
						c.beginPath();
						c.moveTo(x, y);
						c.lineTo(x, Math.min(this.world.h, y + 32));
						c.stroke();
					}
				}
				c.restore();
