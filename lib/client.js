	...
	factory: (require) => {
		...
		function CraftModal({ character, placed, onChange, onClose }) {
			...
			const ringStyle = (i, total) => {
				...
				return { position: 'absolute', left: '50%', top: '50%', zIndex: 2, transform: 'translate(' + (Math.cos(angle) * 78) + 'px,' + (Math.sin(angle) * 78) + 'px) translate(-50%,-50%)', width: 48, height: 48, background: 'rgba(20,22,31,.85)', border: '1px solid #9d6bff', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' };
			};
			return hs('div', { className: 'dsh-vs-cover', children: [
					...
					hs('div', { key: 'body', style: { display: 'flex', gap: 18, minHeight: 300 }, children: [
						hs('div', { key: 'left', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', gap: 8 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 280, overflowY: 'auto' }, children: leftRows.map((r) => {
								return h('div', { key: r.item, className: 'dsh-vs-item', onClick: () => add(r.item), style: { cursor: 'pointer' }, children: [
						...
						hs('div', { key: 'right', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }, children: [
							...
							hs('div', { key: 'ring', style: { position: 'relative', width: 220, height: 220, background: 'rgba(20,22,31,.4)', border: '1px dashed #7c5cfc', borderRadius: '50%', overflow: 'visible' }, children: Array.from({ length: 11 }, (_, i) => {
								const item = placed[i];
