...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function bagEntriesStable(inventory) {
			...
			// 1) 已用完的物品腾空，但记住它原来的格子
			for (let i = 0; i < BAG_SLOTS.length; i++) {
				const slot = BAG_SLOTS[i];
				if (slot && !present.has(slot.item)) {
					BAG_LAST_SLOT.set(slot.item, i);
					BAG_SLOTS[i] = null;
				}
			}
			// 2) 已有物品更新数量，位置不动
			...
			// 3) 新物品优先回到它上次的格子，否则填最早空格，最后追加
			for (const item of present) {
				if (BAG_SLOTS.some((x) => x && x.item === item)) continue;
				let idx = BAG_LAST_SLOT.get(item);
				if (idx == null || idx >= BAG_SLOTS.length || BAG_SLOTS[idx]) {
					idx = BAG_SLOTS.indexOf(null);
				}
				if (idx < 0) idx = BAG_SLOTS.length;
				BAG_SLOTS[idx] = { item, count: counts.get(item) ?? 0 };
				BAG_LAST_SLOT.set(item, idx);
			}
