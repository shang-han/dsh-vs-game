...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-skill-btn{position:absolute;right:10px;bottom:10px;z-index:6;background:rgba(20,22,31,.92);color:#aab0c4;border:1px solid #2a2e3d;border-radius:10px;padding:8px 14px;font-size:12px;font-weight:700;cursor:pointer;pointer-events:auto;}',
		...
		const ACTIVE_SKILLS = {
			strike: {
				...
				icon: '✂️',
				name: '划除',
				desc: '5s 无敌，期间点击屏幕瞬移（最多 6 次），路径留下 5s 划除线灼烧敌人',
				cd: 30,
				duration: 5,
				maxTeleports: 6,
				lineDuration: 5,
		...
		class GameEngine {
			constructor(canvas, opts) {
				...
				this.activeSkillId = 'strike'; // 主动技能；HELLO/CHARACTER 到达后覆盖
			...
			attachInput() {
				...
				this._onPointerDown = (e) => {
					...
					this.tryTeleport(e.clientX - rect.left, e.clientY - rect.top);
				};
			...
			activateActiveSkill() {
				...
				this.setBanner('✂️ ' + sk.name + '！点击屏幕瞬移，最多 ' + sk.maxTeleports + ' 次');
			}
			...
			applyCharacter(c) {
				...
				if (typeof c.activeSkill === 'string' && ACTIVE_SKILLS[c.activeSkill]) this.activeSkillId = c.activeSkill;
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderBag = () => {
				...
				for (let i = 0; i < slotCount; i++) {
					...
					const canOpen = item === 'newbie-gift';
					nodes.push(h('div', {
						key: item,
						className: 'dsh-vs-item' + (canOpen ? ' use' : ''),
						title: meta.name + ' —— ' + meta.desc + (canOpen ? '（点击打开）' : ''),
						onClick: canOpen ? () => send({ kind: ClientMsg.OPEN_ITEM, item }) : undefined,
						children: [
							meta.iconUrl
							...
							canOpen ? h('div', { key: 'a', className: 'dsh-vs-item-action', children: '开' }) : null,
						],
					}));
				}
				return hs('div', { className: 'dsh-vs-inv', children: nodes });
			};
								...
								hs('div', { key: 'skill', className: 'dsh-vs-char-card', children: [
									hs('div', { key: 'main', className: 'dsh-vs-char-card-main', children: [
										...
										hs('div', { key: 't', children: [
											...
											h('div', { key: 's', className: 'dsh-vs-char-card-sub', children: char.activeSkill && ACTIVE_SKILLS[char.activeSkill] ? 'CD ' + ACTIVE_SKILLS[char.activeSkill].cd + 's · 5s无敌 · 最多6次瞬移' : '待开发' }),
										] }),
									] }),
								] }),
							] }),
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'playing' && s.activeSkill ? h('button', {
						...
						children: s.activeSkill.timer > 0
							? (s.activeSkill.icon + ' ' + s.activeSkill.name + ' ' + Math.ceil(s.activeSkill.timer) + 's · 瞬移 ' + s.activeSkill.teleportsLeft + '/' + s.activeSkill.maxTeleports)
							: s.activeSkill.cd > 0
