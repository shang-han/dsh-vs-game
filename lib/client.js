	...
	factory: (require) => {
		...
		function playSE(name, volume = 1, minGap = 0.05) {
			try {
				...
				seLast.set(name, now);
				const url = '/vs-game/assets/music/se/' + name + '.mp3';
						...
						children: [
							hs('div', { key: 'dialog', className: 'dsh-vs-dialog', children: (() => {
								...
								return [hs('div', { key: 'row', style: { display: 'flex', gap: 12, alignItems: 'flex-end' }, children: [
									showPortrait ? h('img', { key: 'portrait', src: '/vs-game/assets/image/tell.png?v=3', alt: '大肥鱼', style: { width: 110, height: 110, objectFit: 'contain', flex: '0 0 auto', filter: 'drop-shadow(0 4px 10px rgba(0,0,0,.5))' } }) : null,
