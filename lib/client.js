...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const MV_ITEM_NAMES = {
			...
			'mat-diamond': '钻石', 'record-player': '唱片机', 'record-billie-jean': 'Billie Jean 唱片', 'record-letmego': 'letmego 唱片', 'record-bad-apple': 'Bad Apple 唱片',
		};
		...
		function loadBadAppleFrames() {
			...
			fetch('/vs-game/assets/music/bgm/bad-apple-frames.bin')
...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			playRecord(item, chestId) {
				...
				const file = item === 'record-billie-jean' ? '/vs-game/assets/music/bgm/Billie Jean.mp3'
					...
					: item === 'record-bad-apple' ? '/vs-game/assets/music/bgm/bad-apple.mp3' : null;
				if (!file) return;
			...
			if (!this._badAppleCanvas || this._badAppleFrameIndex !== fi) {
					const cv = this._badAppleCanvas || document.createElement('canvas');
					cv.width = this.world.w; cv.height = this.world.h;
					const g = cv.getContext('2d');
					const cw = this.world.w / data.cols, ch = this.world.h / data.rows;
					const bpr = Math.ceil(data.cols * data.rows / 8);
					const base = fi * bpr;
					for (let row = 0; row < data.rows; row++) {
						for (let col = 0; col < data.cols; col++) {
							const idx = row * data.cols + col;
							const bit = (data.bytes[base + (idx >> 3)] >> (idx & 7)) & 1;
							g.drawImage(bit ? this._brickLight : this._brickDark, col * cw, row * ch, cw, ch);
						}
					}
					this._badAppleCanvas = cv;
					this._badAppleFrameIndex = fi;
				}
				c.drawImage(this._badAppleCanvas, 0, 0);
			}
		...
		function ShopModal({ character, send, onClose }) {
			...
			const buyPrice = (item) => item === 'mat-wood' ? 200
				...
				: 0;
			const buyRows = ['mat-wood', 'mat-ingot-silver', 'mat-diamond', 'record-billie-jean', 'record-letmego', 'record-bad-apple'].map((item) => ({ item, count: 99, price: buyPrice(item), meta: itemMeta(item), buy: true }));
