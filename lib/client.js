...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-char-cards{display:grid;grid-template-columns:1fr 1fr;gap:8px;}',
			'.dsh-vs-char-card{display:flex;flex-direction:column;align-items:flex-start;justify-content:center;gap:4px;background:#0e1017;border:1px solid #2a2e3d;border-radius:10px;padding:8px 10px;min-width:0;}',
			...
			attachInput() {
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						...
						// 人物界面交互：点家具打开，点地面移动
						const hit = this.homeItems.find((item) => !item.decor && Math.hypot(item.x - x, item.y - y) <= 34);
						if (hit) {
				...
				(character.unlockedSkills ?? []).length === 0
					...
					: hs('div', { key: 'list', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: (character.unlockedSkills ?? []).map((id) => {
						...
						return h('button', {
							...
							children: [
								sk.iconUrl ? h('img', { key: 'i', src: sk.iconUrl, alt: '', style: { width: 16, height: 16, verticalAlign: 'middle', imageRendering: 'pixelated' } }) : sk.icon,
								' ' + sk.name + (active ? '（装备中）' : ''),
							],
						});
					}) }),
					...
					hs('div', { key: 'body', className: 'dsh-vs-char-body', children: [
						hs('div', { key: 'left', className: 'dsh-vs-char-left', children: [
							...
							hs('div', { key: 'cards', className: 'dsh-vs-char-cards', children: [
								hs('div', { key: 'weapon', className: 'dsh-vs-char-card', title: WEAPONS[char.initialWeapon]?.desc ?? '', children: [
									hs('div', { key: 'main', className: 'dsh-vs-char-card-main', children: [
										...
										hs('div', { key: 't', children: [
											...
											h('div', { key: 's', className: 'dsh-vs-char-card-sub', children: '武器' }),
										] }),
									] }),
								] }),
								hs('div', { key: 'skill', className: 'dsh-vs-char-card', title: char.activeSkill && ACTIVE_SKILLS[char.activeSkill] ? ACTIVE_SKILLS[char.activeSkill].desc : '未获得主动技能', children: [
									hs('div', { key: 'main', className: 'dsh-vs-char-card-main', children: [
										...
										hs('div', { key: 't', children: [
											...
											h('div', { key: 's', className: 'dsh-vs-char-card-sub', children: char.activeSkill && ACTIVE_SKILLS[char.activeSkill] ? '技能' : '未获得' }),
										] }),
									] }),
								] }),
							] }),
						...
						hs('div', { key: 'right', className: 'dsh-vs-char-right', children: [
							hs('div', { key: 'tabs', className: 'dsh-vs-char-tabs', children: [
								...
								h('button', { key: 'weapon', className: 'dsh-vs-char-tab' + (tab === 'weapon' ? ' on' : ''), onClick: () => setTab('weapon'), children: '武器' }),
								h('button', { key: 'skill', className: 'dsh-vs-char-tab' + (tab === 'skill' ? ' on' : ''), onClick: () => setTab('skill'), children: '技能' }),
								h('button', { key: 'talent', className: 'dsh-vs-char-tab' + (tab === 'talent' ? ' on' : ''), onClick: () => setTab('talent'), children: '初始天赋' }),
							] }),
							tab === 'bag' ? renderBag() : tab === 'panel' ? renderPanel() : tab === 'weapon' ? renderWeapon() : tab === 'skill' ? renderSkill() : renderTalent(),
						]}),
					]}),
				]}),
				] });
