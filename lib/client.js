...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		function Hud({ snap }) {
			return hs('div', { className: 'dsh-vs-hud', children: [
				...
				snap.levelName ? hs('div', { key: 'tc', className: 'dsh-vs-hud-tc', children: [
					h('span', { key: 'n', children: (snap.levelChapter ? '第 ' + snap.levelChapter + ' 章 · ' : '') + snap.levelName + (snap.compass ? '　🧭 ' + snap.compass.kind + ' ' + snap.compass.dir + ' ' + snap.compass.dist + 'm' + (snap.chestsLeft ? '（余 ' + snap.chestsLeft + '）' : '') : '') }),
					activeLevel?.id === 'busy-server' && (character?.clearedLevels ?? []).includes('busy-server') && snap.phase === 'playing'
						? h('button', { key: 'skip', className: 'dsh-vs-btn ghost', style: { marginLeft: 8, padding: '2px 8px', fontSize: 11, verticalAlign: 'middle' }, onClick: () => engineRef.current?.skipToBoss(), children: '⏩ 直接 Boss' })
						: null,
				] }) : null,
						...
						children: [
							hs('div', { key: 'dialog', className: 'dsh-vs-dialog', children: (() => {
								...
								return [hs('div', { key: 'row', style: { display: 'flex', gap: 12, alignItems: 'flex-end' }, children: [
									showPortrait ? h('img', { key: 'portrait', src: '/vs-game/assets/image/tell.png', alt: '大肥鱼', style: { width: 110, height: 110, objectFit: 'contain', flex: '0 0 auto', filter: 'drop-shadow(0 4px 10px rgba(0,0,0,.5))' } }) : null,
