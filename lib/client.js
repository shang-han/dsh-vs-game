	...
	factory: (require) => {
		...
		class GameEngine {
			...
			enterHome() {
				...
				this.crafting = { stage: 'idle', timer: 0, recipe: [], message: null, messageTimer: 0, x: 150, y: 420 };
		...
		function CraftModal({ character, placed, onChange, onStart, onClose }) {
			...
			const rightRows = [...placedCounts.entries()].map(([item, count]) => ({ item, count, meta: itemMeta(item) }));
			const valid = placed.length === 10 && placed.every((x) => x === 'skill-fragment');
			const add = (item) => {
			...
			const start = () => {
				if (valid) onStart();
				else setMessage('好像不太对');
			};
			const row = (r, onClick, bg, border) => hs('div', { key: r.item, onClick, style: { display: 'flex', alignItems: 'center', gap: 10, background: bg, border: '1px solid ' + border, borderRadius: 8, padding: '8px 10px', cursor: 'pointer' }, children: [
				...
				h('span', { key: 'n', children: r.meta.name }),
				h('span', { key: 'c', className: 'sub', children: '×' + r.count }),
			] });
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 620, minHeight: 440, background: '#0e1017', border: '2px solid #4f6ef7', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 14 }, children: [
					h('h2', { key: 'h', children: '⚒️ 合成仪式' }),
					hs('div', { key: 'body', style: { display: 'flex', gap: 14, minHeight: 280 }, children: [
						hs('div', { key: 'left', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							h('div', { key: 'lh', className: 'sub', children: '背包' }),
							hs('div', { key: 'llist', style: { overflowY: 'auto', maxHeight: 280, display: 'flex', flexDirection: 'column', gap: 6 }, children: leftRows.map((r) => row(r, () => add(r.item), '#1c1f2b', '#2a2e3d')) }),
						] }),
						hs('div', { key: 'right', style: { flex: '1 1 50%', display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							h('div', { key: 'rh', className: 'sub', children: '放置区' }),
							hs('div', { key: 'rlist', style: { overflowY: 'auto', maxHeight: 280, display: 'flex', flexDirection: 'column', gap: 6 }, children: rightRows.map((r) => row(r, () => remove(r.item), '#221a3a', '#7c5cfc')) }),
						] }),
					] }),
					h('div', { key: 'msg', className: 'sub', children: message || (valid ? '配方正确：技能书碎片 ×10 → 技能书·划除' : '点击左侧物品放入，右侧点击可取回') }),
					hs('div', { key: 'btns', style: { display: 'flex', gap: 10, justifyContent: 'center' }, children: [
						h('button', { key: 'start', className: 'dsh-vs-btn', onClick: start, children: '开始仪式' }),
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			const handleHomeAction = (type) => {
				...
				else if (type === 'craft') setCraftOpen(true);
			};
			...
			return hs('div', {
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, placed: craftPlaced, onChange: setCraftPlaced, onStart: () => { setCraftPlaced([]); engineRef.current?.startCraftRitual(); setCraftOpen(false); }, onClose: () => { setCraftOpen(false); engineRef.current?.focusCanvas(); } }) : null,
