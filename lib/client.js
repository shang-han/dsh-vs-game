...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ITEM_META = {
			...
			'skill-book': { icon: '📖', name: '技能书·划除', desc: '使用后学会主动技能「划除」' },
		};
		...
		class GameEngine {
			...
			findBoltTarget(x, y, preferHp = false) {
				...
				for (const e of this.enemies) {
					...
					if (d < bestD) { bestD = d; bestHp = e.hp; best = e; }
				}
				if (!best) return null;
				if (preferHp) {
					// 高等级改为优先追高血量目标
					let hpBest = null, hpVal = -Infinity;
					for (const e of this.enemies) {
						const d = Math.hypot(e.x - x, e.y - y);
						if (d < 700 && e.hp > hpVal) { hpVal = e.hp; hpBest = e; }
					}
					if (hpBest) return hpBest;
				}
				return best;
			}
			...
			fireBolt(lv) {
				...
				const track = lv >= 3 ? 0.4 : lv >= 2 ? 0.18 : 0; // 升级后微跟踪
