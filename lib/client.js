			...
			drawBrickRoom(c) {
				...
				for (let row = 0; row * size < this.world.h; row++) {
					for (let col = 0; col * size < this.world.w; col++) {
						...
						c.save();
						try { c.filter = white ? 'grayscale(1) brightness(2.1) contrast(0.85)' : 'grayscale(1) brightness(0.22) contrast(1.25)'; } catch {}
						if (wall.complete && wall.naturalWidth) {
							c.drawImage(wall, 0, 0, 48, 48, col * size, row * size, size, size);
						} else {
							c.fillStyle = white ? '#e8e8e8' : '#151515';
