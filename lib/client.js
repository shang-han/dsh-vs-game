...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function endlessScale(level) {
			const pre = 1.15 + 0.25 * Math.floor(Math.min(level, 25) / 5);
			if (level <= 25) return pre;
			return pre + 0.15 * (level - 25);
		}
		function endlessXpScale(level) {
			return endlessScale(level) + (level > 25 ? (level - 25) * 0.05 : 0);
		}
		...
		class GameEngine {
			...
			reset() {
				...
				this.cfg = { autoPause: true, autoSelect: false, difficulty: 'normal', idleSpawnRate: 3 };
			...
			spawnEnemyAt(type, x, y, elite) {
				...
				this.enemies.push({
					id: nextId++, type,
					x, y,
					hp: base.hp * hpScale * (elite ? 5 : 1),
					maxHp: base.hp * hpScale * (elite ? 5 : 1),
			...
			spawnBoss(type, hp, areaBomber = false) {
				...
				this.enemies.push({
					...
					areaBomber: !!areaBomber, bombCd: rand(1.6, 2.6),
				});
			...
			spawnEnemy(type, count, elite, meleeElite = false) {
				for (let i = 0; i < count; i++) {
					...
					this.enemies.push({
						...
						hp: base.hp * hpScale * (elite ? 5 : 1),
						maxHp: base.hp * hpScale * (elite ? 5 : 1),
						speed: base.speed * (meleeElite ? 1.6 : elite ? 1.4 : 1),
						size: base.size * (elite ? 1.5 : 1),
						color: base.color, label: base.label,
						xp: base.xp * (elite ? 3 : 1) * xpScale,
						elite: !!elite, meleeElite: !!meleeElite,
						immuneSlow: !!meleeElite, immuneKnockback: !!meleeElite,
			...
			drawAreaBombs(c, t) {
				for (const b of this.areaBombs) {
					...
					if (!b.done) {
						...
						c.strokeStyle = 'rgba(255,152,0,' + (0.5 + 0.4 * k) + ')';
						c.fillStyle = 'rgba(255,152,0,' + (0.10 + 0.10 * k) + ')';
						c.lineWidth = 2;
						c.beginPath();
						c.arc(b.x, b.y, b.r, 0, Math.PI * 2);
						c.fill();
						c.stroke();
						c.fillStyle = 'rgba(255,200,0,.8)';
						c.beginPath();
						c.arc(b.x, b.y, 4, 0, Math.PI * 2);
						c.fill();
					} else {
						const k = Math.max(0, b.life / 0.9);
						c.strokeStyle = 'rgba(255,152,0,' + (0.7 * k) + ')';
						c.lineWidth = 3;
						c.beginPath();
			...
			updateEnemies(dt) {
				...
				for (const e of this.enemies) {
					...
					// 区域轰炸 Boss：瞄准玩家附近区域，落点有预警圈
					if (e.areaBomber) {
						e.bombCd -= dt;
						if (e.bombCd <= 0) {
							e.bombCd = rand(2.6, 4.0);
							const tx = Math.max(20, Math.min(this.world.w - 20, this.player.x + rand(-90, 90)));
							const ty = Math.max(20, Math.min(this.world.h - 20, this.player.y + rand(-90, 90)));
			...
			checkLevelUp() {
				...
				while (p.xp >= p.xpNeed) {
					...
					if (this.cfg.autoSelect) {
						...
						this.phase = 'levelup';
						this.applyChoice(Math.floor(Math.random() * choices.length), true);
						continue;
					}
			...
			return hs('div', {
				...
				cfgOpen && cfg ? hs('div', { key: 'pop', className: 'dsh-vs-pop', children: [
					...
					h('label', { key: 'as', children: [
						h('span', { key: 't', children: '升级时自动随机选一' }),
