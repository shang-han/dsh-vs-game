...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		function ShopModal({ character, send, onClose }) {
			...
			const buyPrice = (item) => item === 'mat-wood' ? 200
				...
				: 0;
			const buyRows = ['mat-wood', 'mat-ingot-silver', 'mat-diamond', 'record-billie-jean', 'record-letmego', 'record-bad-apple', 'record-world-execute-me'].map((item) => ({ item, count: 99, price: buyPrice(item), meta: itemMeta(item), buy: true }));
