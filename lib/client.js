	...
	factory: (require) => {
		...
		class GameEngine {
			...
			updateHome(dt) {
				...
				if (craftNear && this.focused && this.keys.has('f') && this.homeFCooldown <= 0) {
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, placed: craftPlaced, onChange: (items) => { setCraftPlaced(items); engineRef.current?.setCraftPlaced(items); }, onStart: () => { engineRef.current?.startCraftRitual(); setCraftOpen(false); }, onClose: () => { setCraftOpen(false); engineRef.current?.focusCanvas(); } }) : null,
