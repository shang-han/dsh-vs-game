...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ClientMsg = {
			...
			PLACE_CHEST: 'place-chest', CHEST_TRANSFER: 'chest-transfer', SET_CRAFTING_STORAGE: 'set-crafting-storage',
		};
		...
		class GameEngine {
			...
			attachInput() {
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						...
						if (hit) { if (hit.id === 'craft') { if (this.crafting?.stage === 'done') this.handleCraftInteraction(); else this.onHomeAction('craft'); } else this.onHomeAction(hit.id); return; }
			...
			updateHome(dt) {
				...
				const craftNear = this.homeNear?.id === 'craft';
				if (this.focused && this.keys.has('f') && this.homeFCooldown <= 0) {
					...
					this.fDown = false;
					this.fLongTriggered = false;
					if (!wasLong && craftNear) this.onHomeAction('craft');
				}
			}
		...
		function CraftModal({ character, placed, onChange, onStart, onClose }) {
			...
			const leftRows = [...totals.entries()].map(([item, total]) => ({ item, total, placed: placedCounts.get(item) ?? 0, meta: itemMeta(item) })).filter((r) => r.placed < r.total);
			const add = (item) => {
				if (placed.length >= 11) return;
				const total = totals.get(item) ?? 0;
				const cur = placedCounts.get(item) ?? 0;
				if (cur < total) onChange([...placed, item]);
			};
			const remove = (index) => {
				const next = [...placed];
				next.splice(index, 1);
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, minHeight: 300 }, children: [
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 280, overflowY: 'auto' }, children: leftRows.map((r) => {
								...
								(r.total - r.placed) > 1 ? h('div', { key: 'c', className: 'dsh-vs-item-count', children: '×' + (r.total - r.placed) }) : null,
					...
					nodes.push(h('div', {
						...
						children: [
							...
							canOpen && isSel ? h('button', {
								...
								onClick: (e) => {
									e.stopPropagation();
									send({ kind: ClientMsg.PLACE_CHEST, item });
									setSelectedItem(null);
								},
								children: '放置',
							}) : null,
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				charOpen ? h(CharacterModal, { key: 'char', character, goldEarned, send, onClose: () => { setCharOpen(false); engineRef.current?.focusCanvas(); } }) : null,
			] });
		}
