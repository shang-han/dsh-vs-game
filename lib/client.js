...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ClientMsg = {
			...
			CRAFT_ITEM: 'craft-item', SELL_ITEM: 'sell-item', BUY_ITEM: 'buy-item',
			EQUIP_ACCESSORY: 'equip-accessory', UNEQUIP_ACCESSORY: 'unequip-accessory',
			PLACE_CHEST: 'place-chest', EJECT_CONTAINER_ITEM: 'eject-container-item', REMOVE_CHEST: 'remove-chest', PICKUP_GROUND: 'pickup-ground', CHEST_TRANSFER: 'chest-transfer', SET_CRAFTING_STORAGE: 'set-crafting-storage',
		};
		...
		const PASSIVES = {
			...
			might:  { icon: '💪', name: '力量', desc: '全伤害 +20% / 级', per: '全伤害 +20%', current: (lv) => '全伤害 +' + lv * 20 + '%' },
		...
		const ACTIVE_SKILLS = {
			...
			'teleport-laser': {
				...
				name: 'TP',
				desc: '点击屏幕发射传送激光，抵达落点后传送过去；无技能CD，仅有内置发射间隔',
				cd: 0,
				type: 'teleport',
				internalCd: 0.35,
				laserSpeed: 1500,
			...
			'damage-laser': {
				...
				icon: '🔫',
				name: '伤害激光',
				desc: '点击屏幕发射伤害激光，命中落点造成范围伤害；无技能CD，仅有内置发射间隔',
				cd: 0,
				type: 'damage',
				internalCd: 0.28,
				laserSpeed: 1800,
		...
		const ITEM_DEFS = {
			...
			'record-letmego': { icon: '💿', iconUrl: MV_ICON_BASE + 'record.png', name: 'letmego 唱片', type: 'record', desc: 'BGM：letmego' },
		...
		function itemMeta(item) {
			if (ITEM_DEFS[item]) return ITEM_DEFS[item];
			if (MV_ITEM_NAMES[item]) {
				const isAcc = item.startsWith('acc-');
				if (item === 'mat-wood') return { icon: '🪵', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], type: 'material', desc: '基础合成材料' };
				if (item === 'wooden-chest') return { icon: '📦', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], type: 'tool', desc: '可放置的 5 格储物箱' };
				if (isAcc) return { icon: '💍', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], type: 'accessory', dev: true, placeholder: true, desc: 'AI 生成的占位饰品，功能尚未实现。' };
				return { icon: '🧱', iconUrl: MV_ICON_BASE + item + '.png', name: MV_ITEM_NAMES[item], type: 'material', desc: '锻造材料（打造高级饰品）' };
			}
					...
					: m.type === 'material' ? '材料'
						: m.type === 'consumable' ? '消耗品'
							: m.type === 'tool' ? '工具'
								: '物品';
			const stats = [];
			if (!dev && Array.isArray(m.attack)) stats.push({ label: '攻击', value: m.attack[0] + ' - ' + m.attack[1], tone: 'good' });
			const sockets = [];
			if (!dev && m.type === 'accessory' && quality) {
				for (let i = 0; i < quality.sockets; i++) sockets.push({ index: i, affix: null });
			}
			return { item, name: m.name, icon: m.icon, iconUrl: m.iconUrl, quality, typeLabel, stats, sockets, desc: m.desc, dev };
		}
		function ItemCardBody({ item }) {
			...
			return hs('div', { children: [
				hs('div', { key: 'h', style: { display: 'flex', alignItems: 'center', gap: 8 }, children: [
					d.iconUrl ? h('img', { key: 'i', src: d.iconUrl, alt: d.name, style: { width: 32, height: 32, imageRendering: 'pixelated', objectFit: 'contain' } }) : h('span', { key: 'i', style: { fontSize: 24 }, children: d.icon }),
				...
				d.sockets.length ? h('div', { key: 'so', style: { marginTop: 8, display: 'flex', flexDirection: 'column', gap: 2 }, children: d.sockets.map((s, i) => h('div', { key: i, style: { fontSize: 13, lineHeight: '1.5', color: '#8a8fa3' }, children: s.affix ? s.affix.label : '◇' })) }) : null,
...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			enterHome(focus = true) {
				...
				this.crafting = { stage: 'idle', timer: 0, recipe: [], placed: [], result: null, message: null, messageTimer: 0, x: 150, y: 420 };
			...
			appendHomeContainers(items) {
				for (let i = 0; i < (this.homeChests ?? []).length; i++) {
					...
					items.push({ id: 'chest:' + i, x: pos.x, y: pos.y, icon: '📦', label: kind === 'record-player' ? '唱片机' : '木制宝箱', noEmoji: true });
				}
			...
			buildHomeItems() {
				if (this.homeRoom === 1) {
					const items = [
						{ id: 'room-unused', x: 60, y: 450, icon: '🚪', label: '' },
			...
			homeMessage(text) {
				if (this.crafting) { this.crafting.message = text; this.crafting.messageTimer = 1.6; }
			}
			...
			enterHomeRoom(n) {
				...
				this.homeMoveTarget = null;
				this.homeNear = null;
				this.player.x = this.homeRoom === 1 ? 420 : 220;
				this.player.y = this.homeRoom === 1 ? 360 : 330;
				this.homeItems = this.buildHomeItems();
			}
			...
			playRecord(item, chestId) {
				...
				const file = item === 'record-billie-jean' ? '/vs-game/assets/music/bgm/Billie Jean.mp3'
					: item === 'record-letmego' ? '/vs-game/assets/music/bgm/letmego.mp3'
			...
			pickupGround(id) {
				...
				if (g) {
					...
					if (!unique.has(g.item) && unique.size >= 24) {
						if (this.crafting) { this.crafting.message = '已经满了'; this.crafting.messageTimer = 1.6; }
						return;
					}
				}
				if (g && this.crafting) {
			...
			spawnEnemyAt(type, x, y, elite) {
				...
				if (elite) playSE('elite-spawn', 0.7, 0.1);
				...
				this.enemies.push({
					id: nextId++, type,
					x, y,
					hp: base.hp * hpScale * (elite ? 20 : 1),
					maxHp: base.hp * hpScale * (elite ? 20 : 1),
					speed: base.speed * (elite ? 1.4 : 1), size: base.size * (elite ? 1.5 : 1),
					color: base.color, label: base.label,
					xp: base.xp * (elite ? 3 : 1),
					elite: !!elite, immuneKnockback: !!elite, hitFlash: 0, slow: 0, aggro: false,
			...
			attachInput() {
				...
				this._onKeyDown = (e) => {
					...
					if (['w', 'a', 's', 'd', 'arrowup', 'arrowdown', 'arrowleft', 'arrowright', ' ', '1', '2', '3', 'e', 'f', 'b', 'p', 'escape'].includes(k)) {
						e.preventDefault();
						e.stopPropagation();
					}
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						...
						// 激光技能：点击屏幕发射，家里也能用
						if (this.isLaserSkill() && this.laserSkillOn) { this.fireLaserSkill(x, y); return; }
						// 划除：家里也能点击快速移动
						if (this.skillTimer > 0 && ACTIVE_SKILLS[this.activeSkillId]?.id === 'strike') { this.tryDash(x, y); return; }
						// 人物界面交互：点家具打开，点地面移动
						const hit = this.homeItems.find((item) => Math.hypot(item.x - x, item.y - y) <= 34);
						if (hit) {
							...
							else if (hit.id === 'room-unused') this.homeMessage('未启用');
				...
				this._onUiClick = (e) => {
					const t = e.target;
					if (t && t.closest && t.closest('button, .dsh-vs-card, .dsh-vs-item, .dsh-vs-chip')) playSE(UI_CLICK_SOUNDS[Math.floor(Math.random() * UI_CLICK_SOUNDS.length)], 0.45, 0.03);
				};
			...
			updateHome(dt) {
				...
				if (this.focused && this.keys.has('f') && this.homeFCooldown <= 0) {
					if (craftNear) {
						...
						if (!this.fLongTriggered && performance.now() - this.fStart > 450) {
							this.fLongTriggered = true;
							this.onHomeAction('chest-menu:' + nearId.slice(6));
						}
			...
			updateCrafting(dt) {
				...
				if (c.messageTimer > 0) {
					c.messageTimer -= dt;
					if (c.messageTimer <= 0) c.message = null;
				}
				if (c.stage === 'crafting') {
					c.timer -= dt;
					if (c.timer <= 0) {
			...
			fireLaserSkill(tx, ty) {
				...
				this.skillLasers.push({
					...
					type: sk.type, radius: sk.radius ?? 48,
					damage: this.attackPower() * (sk.damageMult ?? 1),
				});
				playSE('laser-shot', 0.65, 0.03);
				this.burst(this.player.x, this.player.y, sk.type === 'teleport' ? '#b46bff' : '#ff3b3b', 8);
				return true;
			}

			enderBurst(x, y) {
				this.burst(x, y, '#b46bff', 22);
				for (let i = 0; i < 8; i++) {
					this.particles.push({ kind: 'ender', x, y, life: 0.35 + i * 0.03, maxLife: 0.62, color: '#b46bff', size: 12 + i * 5 });
				}
			}
			...
			updateSkillLasers(dt) {
				...
				for (const l of this.skillLasers) {
					...
					if (!l.done && l.t >= l.dur) {
						...
						if (l.type === 'teleport') {
							...
							this.burst(l.x2, l.y2, '#ff3b3b', 18);
							for (const e of [...this.enemies]) {
								if (Math.hypot(e.x - l.x2, e.y - l.y2) <= l.radius + e.size * 0.4) {
				...
				this.skillLasers = this.skillLasers.filter((l) => l.t < l.dur + 0.18);
			}
			...
			drawSkillLasers(c) {
				...
				for (const l of this.skillLasers) {
					...
					const isTp = l.type === 'teleport';
					c.save();
					if (isTp) {
						// 末影珍珠飞行：只留残影，不画首尾连线
						for (let i = 1; i <= 3; i++) {
							const kk = Math.max(0, k - i * 0.07);
							const tx = l.x1 + (l.x2 - l.x1) * kk;
							const ty = l.y1 + (l.y2 - l.y1) * kk;
							c.globalAlpha = 0.28 / i;
							c.fillStyle = '#b46bff';
							c.beginPath(); c.arc(tx, ty, 7, 0, Math.PI * 2); c.fill();
						}
						c.globalAlpha = 1;
			...
			applyCharacter(c) {
				...
				if (nextActiveSkill !== this.activeSkillId) this.laserSkillOn = false;
			...
			spawnEnemy(type, count, elite, meleeElite = false) {
				...
				for (let i = 0; i < count; i++) {
					...
					this.enemies.push({
						...
						hp: base.hp * hpScale * eliteHpMul,
						maxHp: base.hp * hpScale * eliteHpMul,
						speed: base.speed * (meleeElite ? (lateMelee ? 3.6 : 1.8) : elite ? 1.4 : 1),
			...
			idleSpawn(dt) {
				...
				if (this.elapsed > 180 && this.player.level >= 15 && Math.random() < 0.012 && !this.enemies.some((e) => e.areaBomber)) {
					this.spawnBoss(pick(['bin', 'term', 'rs']), Math.round(100 + this.elapsed * 0.8) * this.diffMul(), true);
				}
			}
			...
			critChance() { return BASE_CRIT_RATE; }
			critDamage() { return BASE_CRIT_DMG; }
			rollDamage(dmg) {
				if (Math.random() < this.critChance()) return { dmg: dmg * this.critDamage(), crit: true };
				return { dmg, crit: false };
			}
			defensePower() {
				let def = (this.player.passives.armor || 0) * 5;
			...
			fireBoltShot() {
				...
				if (!t) return;
				playSE('token-shot', 0.5, 0.04);
				const baseAngle = Math.atan2(t.y - this.player.y, t.x - this.player.x);
				...
				for (const b of this.beams) {
					...
					for (const e of this.grid.query(b.x, b.y, b.len)) {
						...
						if (perp < b.width / 2 + e.size * 0.5) {
							...
							this.hurtEnemy(e, b.baseDmg ?? b.damage ?? this.weaponDamage('laser', 1));
						}
					}
				}
				// 护盾/主动技能无敌期间不吃任何伤害
				...
				if (p.invuln <= 0) {
					for (const e of this.grid.query(p.x, p.y, 40)) {
						if (Math.hypot(e.x - p.x, e.y - p.y) < e.size * 0.55 + 13) {
							const raw = (8 + Math.min(20, this.elapsed / 30) + (e.elite ? 4 : 0)) * this.diffMul();
				...
				if (p.invuln <= 0) {
					for (const b of this.enemyBullets) {
						...
						if (Math.abs(b.x - p.x) <= hw && Math.abs(b.y - p.y) <= hh) {
							b.life = 0;
							const dmg = this.incomingDamage((6 + Math.min(12, this.elapsed / 40)) * this.diffMul());
							p.hp -= dmg;
							playSE('player-hurt', 0.75, 0.12);
							p.invuln = 0.5;
			...
			drawHomeObjects(c, t) {
				...
				for (let i = 0; i < (this.homeChests ?? []).length; i++) {
					...
					const pos = this.chestHomePos(i);
					const kind = this.homeChests[i]?.kind;
					const img = assetImg(kind === 'record-player' ? '/vs-game/assets/items/mv/record-player.png' : '/vs-game/assets/items/mv/wooden-chest.png');
				...
				if (this.crafting?.message) {
					...
					c.shadowColor = '#000';
					c.shadowBlur = 4;
					c.fillText(this.crafting.message, p.x, p.y - 60 - (1.6 - this.crafting.messageTimer) * 30);
					c.shadowBlur = 0;
					c.globalAlpha = 1;
				}
			}
			...
			drawHomeItems(c, t) {
				...
				c.textBaseline = 'middle';
				for (const item of this.homeItems) {
					const bob = (item.id === 'door' || item.id.startsWith('room')) ? 0 : Math.sin(t * 2.2 + item.x) * 3;
					...
					c.shadowColor = '#000';
					c.shadowBlur = 4;
					c.fillText(item.label, item.x, item.y + bob + 40);
					c.shadowBlur = 0;
				}
				if (this.homeNear && this.crafting?.stage !== 'crafting') {
			...
			drawPlayer(c, t) {
				...
				let spriteT = t;
				if ((state === 'strike' || state === 'shoot') && p.skillAction) {
					spriteT = p.skillAction.t;
				}
				const drawn = drawSprite(c, state, spriteT, p.x, p.y, 56, flip);
				if (!drawn) {
					c.fillStyle = '#7c5cfc';
					c.beginPath();
					...
					hs('div', { key: 'box', className: 'dsh-vs-gallery-box', children: [
						...
						hs('div', { key: 'grid', className: 'dsh-vs-gallery-grid', children: arts.map((lv) => {
							...
							return hs('div', { key: lv.id, className: 'dsh-vs-gallery-card' + (unlocked ? '' : ' locked'), children: [
								unlocked
									? h('img', { key: 'i', src: '/vs-game/' + lv.story.artPost, alt: lv.name })
		...
		function CraftModal({ character, placed, onChange, onStart, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
						...
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 8, minWidth: 0 }, children: [
							...
							hs('div', { key: 'grid', className: 'dsh-vs-inv', style: { maxHeight: 280, overflowY: 'auto' }, children: leftRows.map((r) => {
								...
								return h('div', { key: r.slot, className: 'dsh-vs-item', ...tipProps(r.item), onClick: () => add(r.item), children: [
									r.meta.iconUrl ? h('img', { key: 'i', className: 'dsh-vs-item-img', src: r.meta.iconUrl, alt: r.meta.name }) : h('div', { key: 'i', className: 'dsh-vs-item-icon', children: r.meta.icon }),
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, minWidth: 0 }, children: [
							...
							hs('div', { key: 'ring', style: { position: 'relative', width: 190, height: 190, background: 'rgba(20,22,31,.4)', border: '1px dashed #7c5cfc', borderRadius: '50%', overflow: 'visible' }, children: [
								...Array.from({ length: 11 }, (_, i) => {
									const item = placed[i];
									return h('div', { key: i, ...(item ? tipProps(item) : {}), onClick: () => { if (item) remove(i); }, title: item ? '点击取回' : '', style: ringStyle(i, 11), children: item ? (() => { const meta = itemMeta(item); return meta.iconUrl ? h('img', { key: 'ii', src: meta.iconUrl, style: { width: 26, height: 26, imageRendering: 'pixelated' } }) : h('span', { key: 'ic', style: { fontSize: 20 }, children: meta.icon }); })() : null });
								}),
				...
				/** 长按宝箱：打开 / 拆除 */
		function ChestActionModal({ onOpen, onRemove, onClose, title = '📦 木制宝箱', removeLabel = '拆除（返还 1 木材）' }) {
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 260, background: '#0e1017', border: '2px solid #8a6b3d', borderRadius: 14, padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }, children: [
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }, children: [
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }, children: [
							...
							h('div', { key: 'slot', className: 'dsh-vs-item' + (slot ? ' use' : ' empty'), onClick: () => { if (slot && container) { onStop(); send({ kind: ClientMsg.CHEST_TRANSFER, chestId: container.id, direction: 'out', slot: 0 }); } }, style: { width: 72, height: 72 }, children: playable ? [
								playable.iconUrl ? h('img', { key: 'i', className: 'dsh-vs-item-img', src: playable.iconUrl, alt: playable.name }) : h('div', { key: 'i', className: 'dsh-vs-item-icon', children: playable.icon }),
		...
		function ChestModal({ character, index, send, onClose }) {
			...
			const slots = Array.isArray(chest?.slots) ? chest.slots : [null, null, null, null, null];
				...
				hs('div', { key: 'panel', style: { width: 660, minHeight: 420, background: '#0e1017', border: '2px solid #8a6b3d', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }, children: [
					...
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }, children: [
						...
						hs('div', { key: 'right', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [
							h('div', { key: 'rh', className: 'sub', children: '箱子（点击取出）5 格' }),
							hs('div', { key: 'slots', style: { display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8 }, children: slots.map((slot, i) => {
								...
								return h('div', { key: i, className: 'dsh-vs-item' + (slot ? ' use' : ' empty'), ...(slot && slot.item ? tipProps(slot.item) : {}), onClick: () => { if (slot) send({ kind: ClientMsg.CHEST_TRANSFER, chestId: chest.id, direction: 'out', slot: i }); }, children: meta ? [
									meta.iconUrl ? h('img', { key: 'i', className: 'dsh-vs-item-img', src: meta.iconUrl, alt: meta.name }) : h('div', { key: 'i', className: 'dsh-vs-item-icon', children: meta.icon }),
		...
		function ShopModal({ character, send, onClose }) {
			useEffect(() => {
			...
			const price = (item) => item === 'mat-wood' ? 100
				...
				: item === 'skill-fragment' ? 50
				: item === 'skill-fragment-teleport' || item === 'skill-fragment-damage' ? 500
				: item.startsWith('skill-fragment') ? 50
			...
			const buyPrice = (item) => item === 'mat-wood' ? 200
				...
				: item === 'skill-fragment-teleport' || item === 'skill-fragment-damage' ? 1000
				: 0;
			const buyRows = ['mat-wood', 'mat-ingot-silver', 'mat-diamond', 'record-billie-jean', 'record-letmego', 'record-bad-apple', 'record-world-execute-me', 'skill-fragment-teleport', 'skill-fragment-damage'].map((item) => ({ item, count: 99, price: buyPrice(item), meta: itemMeta(item), buy: true }));
			...
			const rows = tab === 'sell' ? sellRows : buyRows;
			...
			const confirm = () => {
				if (!current) return;
				const q = Math.max(1, Math.min(qty || 1, current.count));
				...
				hs('div', { key: 'panel', style: { width: 620, minHeight: 460, backgroundImage: 'url("/vs-game/assets/ui/shop.png")', backgroundSize: '110% 110%', backgroundPosition: 'center', borderRadius: 18, padding: 34, display: 'flex', flexDirection: 'column', gap: 12, color: '#e6e8f0' }, children: [
					...
					hs('div', { key: 'body', style: { display: 'flex', gap: 14, minHeight: 250, alignItems: 'stretch' }, children: [
						hs('div', { key: 'list', style: { flex: '1 1 56%', overflowY: 'auto', maxHeight: 290, display: 'flex', flexDirection: 'column', gap: 6 }, children: rows.length === 0
							? [h('div', { key: 'empty', className: 'dsh-vs-empty', children: tab === 'sell' ? '没有可卖出的物品' : '暂无可购买商品' })]
							: rows.map((r) => {
								const isSel = selected?.item === r.item;
								return hs('div', { key: r.item, ...tipProps(r.item), onClick: () => pick(r), style: { display: 'flex', alignItems: 'center', gap: 10, background: isSel ? 'rgba(79,110,247,.22)' : 'rgba(10,12,18,.65)', border: '1px solid ' + (isSel ? '#4f6ef7' : '#2a2e3d'), borderRadius: 8, padding: '8px 12px', cursor: 'pointer' }, children: [
									h('span', { key: 'i', children: r.meta.iconUrl ? h('img', { key: 'ii', src: r.meta.iconUrl, style: { width: 26, height: 26, imageRendering: 'pixelated' } }) : r.meta.icon }),
									h('span', { key: 'n', children: r.meta.name + (r.count > 1 ? ' ×' + r.count : '') }),
									h('span', { key: 'p', className: 'sub', children: (r.buy ? '-' : '+') + r.price + ' 金币' }),
								] });
							}) }),
						hs('div', { key: 'detail', style: { flex: '0 0 40%', background: 'rgba(10,12,18,.65)', border: '1px solid #2a2e3d', borderRadius: 10, padding: 14, display: 'flex', flexDirection: 'column', gap: 10 }, children: current ? [
							h('div', { key: 'di', children: current.meta.iconUrl ? h('img', { key: 'ii', src: current.meta.iconUrl, style: { width: 42, height: 42, imageRendering: 'pixelated' } }) : current.meta.icon }),
							...
							hs('div', { key: 'qrow', style: { display: 'flex', alignItems: 'center', gap: 8 }, children: [
								...
								h('input', { key: 'qi', type: 'number', min: 1, max: current.count, value: qty, onChange: (e) => setQty(Math.max(1, Math.min(current.count, Number(e.target.value) || 1))), style: { width: 64, background: '#0e1017', color: '#e6e8f0', border: '1px solid #2a2e3d', borderRadius: 6, padding: '4px 6px' } }),
						...
						] : [h('div', { key: 'tip', className: 'dsh-vs-empty', children: tab === 'sell' ? '点击左侧物品，右侧设置卖出数量' : '点击左侧物品，右侧设置购买数量' })] }),
					] }),
		...
		function CharacterModal({ character, goldEarned, send, onClose, onPlaceChest }) {
			...
			const renderBag = () => {
				...
				for (let i = 0; i < slotCount; i++) {
					...
					const meta = itemMeta(item);
					const canOpen = item === 'newbie-gift' || item === 'skill-book' || item === 'skill-book-teleport' || item === 'skill-book-damage';
			...
			const renderPanel = () => {
				...
				return hs('div', { className: 'dsh-vs-char-panel', children: [
					h('div', { key: 'hp', className: 'dsh-vs-upgrade-row', children: [
						...
						h('span', { key: 'v', className: 'dsh-vs-upgrade-cost', children: '100' }),
					] }),
					...
					h('div', { key: 'crit', className: 'dsh-vs-upgrade-row', children: [
						...
						h('span', { key: 'v', className: 'dsh-vs-upgrade-cost', children: (BASE_CRIT_RATE * 100).toFixed(0) + '%' }),
					] }),
					h('div', { key: 'critdmg', className: 'dsh-vs-upgrade-row', children: [
						h('span', { key: 'k', className: 'dsh-vs-upgrade-name', children: '暴击伤害' }),
						h('span', { key: 'v', className: 'dsh-vs-upgrade-cost', children: BASE_CRIT_DMG.toFixed(1) + 'x' }),
					] }),
				] });
			};
					...
					hs('div', { key: 'body', className: 'dsh-vs-char-body', children: [
						hs('div', { key: 'left', className: 'dsh-vs-char-left', children: [
							...
							hs('div', { key: 'cards', className: 'dsh-vs-char-cards', children: [
								...
								hs('div', { key: 'skill', className: 'dsh-vs-char-card', children: [
									hs('div', { key: 'main', className: 'dsh-vs-char-card-main', children: [
										char.activeSkill && ACTIVE_SKILLS[char.activeSkill]?.iconUrl
											? h('img', { key: 'i', src: ACTIVE_SKILLS[char.activeSkill].iconUrl, alt: '', style: { width: 20, height: 20, imageRendering: 'pixelated' } })
								...
								h('div', { key: 'skill-list', style: { display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }, children: (character.unlockedSkills ?? []).map((id) => {
									const sk = ACTIVE_SKILLS[id];
									if (!sk) return null;
									const active = character.activeSkill === id;
									return h('button', {
										key: id,
										className: 'dsh-vs-mini-btn' + (active ? ' on' : ''),
										onClick: () => send({ kind: ClientMsg.EQUIP_SKILL, skill: id }),
										children: [
											sk.iconUrl ? h('img', { key: 'i', src: sk.iconUrl, alt: '', style: { width: 14, height: 14, verticalAlign: 'middle', imageRendering: 'pixelated' } }) : sk.icon,
											' ' + sk.name + (active ? '（装备中）' : ''),
										],
									});
								}) }),
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'home' && s.activeSkill ? h('button', {
						key: 'homeSkill',
						className: 'dsh-vs-skill-btn has-e' + (s.activeSkill.type === 'strike' && s.activeSkill.timer > 0 ? ' on' : ''),
						onClick: () => engineRef.current?.activateActiveSkill(),
						children: s.activeSkill.iconUrl
							? [h('img', { key: 'i', src: s.activeSkill.iconUrl, alt: '', style: { width: 16, height: 16, verticalAlign: 'middle', imageRendering: 'pixelated' } }), ' ' + s.activeSkill.name + (s.activeSkill.type === 'strike' ? (s.activeSkill.timer > 0 ? ' ' + Math.ceil(s.activeSkill.timer) + 's' : '（E）') : (s.activeSkill.enabled ? ' 已开启' : ' 已关闭'))]
							: (s.activeSkill.icon + ' ' + s.activeSkill.name + (s.activeSkill.type === 'strike' ? (s.activeSkill.timer > 0 ? ' ' + Math.ceil(s.activeSkill.timer) + 's' : '（E）') : (s.activeSkill.enabled ? ' 已开启' : ' 已关闭'))),
					}) : null,
					...
					s && s.phase === 'playing' && s.activeSkill ? h('button', {
						...
						className: 'dsh-vs-skill-btn has-e' + (s.activeSkill.type === 'strike' && s.activeSkill.timer > 0 ? ' on' : ''),
						onClick: () => engineRef.current?.activateActiveSkill(),
						children: s.activeSkill.type !== 'strike'
							? [
								s.activeSkill.iconUrl ? h('img', { key: 'si', src: s.activeSkill.iconUrl, alt: '', style: { width: 16, height: 16, verticalAlign: 'middle', imageRendering: 'pixelated' } }) : s.activeSkill.icon,
						...
						children: [
							hs('div', { key: 'dialog', className: 'dsh-vs-dialog', children: (() => {
								...
								return [hs('div', { key: 'row', style: { display: 'flex', gap: 12, alignItems: 'flex-end' }, children: [
									showPortrait ? h('img', { key: 'portrait', src: '/vs-game/assets/image/tell.png?v=4', alt: '大肥鱼', style: { width: 110, height: 110, objectFit: 'contain', flex: '0 0 auto', filter: 'drop-shadow(0 4px 10px rgba(0,0,0,.5))' } }) : null,
				...
				shopOpen ? h(ShopModal, { key: 'shop', character, send, onClose: () => { setShopOpen(false); engineRef.current?.focusCanvas(); } }) : null,
				...
				chestMenu != null ? h(ChestActionModal, { key: 'chestmenu',
					title: character?.chests?.[chestMenu]?.kind === 'record-player' ? '🎵 唱片机' : '📦 木制宝箱',
					removeLabel: character?.chests?.[chestMenu]?.kind === 'record-player' ? '拆除' : '拆除（返还 1 木材）',
					...
					onClose: () => { setChestMenu(null); engineRef.current?.focusCanvas(); } }) : null,
