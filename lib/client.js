...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			placeContainerAt(x, y) {
				...
				this.sendWs({ kind: ClientMsg.PLACE_CHEST, placeItem: this.placingChestItem || 'wooden-chest', containerKind: this.placingChestKind || 'chest', x: px, y: py });
				this.placingChest = false;
			}
		...
		function ChestActionModal({ onOpen, onRemove, onClose, title = '📦 木制宝箱', removeLabel = '拆除（返还 1 木材）' }) {
			return hs('div', { className: 'dsh-vs-cover', children: [
				...
				/** 唱片机：左侧唱片，右侧一个播放槽 */
		function RecordPlayerModal({ character, index, send, onPlay, onPause, onResume, onClose }) {
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				recordOpen != null ? h(RecordPlayerModal, { key: 'record', character, index: recordOpen, send, onPlay: (item, chestId) => engineRef.current?.playRecord(item, chestId), onPause: () => engineRef.current?.pauseRecord(), onResume: () => engineRef.current?.resumeRecord(), onClose: () => { setRecordOpen(null); engineRef.current?.focusCanvas(); } }) : null,
