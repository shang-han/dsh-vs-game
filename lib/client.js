...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		class GameEngine {
			...
			buildHomeItems() {
				const items = [
					{ id: 'bag', x: 150, y: 220, icon: '🎒', label: '背包 / 角色' },
					{ id: 'dex', x: 330, y: 120, icon: '📚', label: '图鉴' },
					{ id: 'gallery', x: 520, y: 120, icon: '🖼️', label: '画廊' },
					{ id: 'settings', x: 150, y: 420, icon: '⚙️', label: '设置' },
			...
			attachInput() {
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						...
						if (hit) { this.onHomeAction(hit.id); return; }
			...
			updateHome(dt) {
				...
				if (this.homeNear && this.focused && this.keys.has('f') && this.homeFCooldown <= 0) {
			...
			attackPower() { return BASE_ATTACK * this.dmgMul(); }
			...
			drawHome(c, t) {
				// 家：使用 tileset 铺地板和墙面，未加载完成时回退纯色
				...
				// 左右墙（A4 第一格）
				if (wall.complete && wall.naturalWidth) {
					for (let y = 96; y < this.world.h; y += 48) {
						c.drawImage(wall, 0, 0, 48, 48, 0, y, 48, 48);
						c.drawImage(wall, 0, 0, 48, 48, this.world.w - 48, y, 48, 48);
					}
				}
				// 地毯（Inside_C Rugs A）
				const rug = tileImg('Inside_C');
				if (rug.complete && rug.naturalWidth) {
					c.drawImage(rug, 12 * 48, 6 * 48, 48, 48, 250, 285, 170, 110);
				} else {
					c.fillStyle = 'rgba(124,92,252,.18)';
					c.fillRect(260, 280, 300, 170);
					c.strokeStyle = '#7c5cfc';
					c.strokeRect(260, 280, 300, 170);
				}
				// 窗户
				c.fillStyle = '#20303a';
				c.fillRect(640, 20, 90, 60);
				c.strokeStyle = '#8a8fa3';
				c.strokeRect(640, 20, 90, 60);
				c.beginPath();
				c.moveTo(685, 20); c.lineTo(685, 80);
				c.moveTo(640, 50); c.lineTo(730, 50);
				c.stroke();
				c.fillStyle = 'rgba(255,255,255,.85)';
				c.font = '24px serif';
				c.textAlign = 'center';
				c.fillText('🐟', this.world.w / 2, 60);
				// 右侧大门
				c.fillStyle = '#26160e';
				c.fillRect(705, 160, 90, 260);
				c.fillStyle = '#4a2e1d';
				c.fillRect(715, 170, 70, 240);
				c.fillStyle = '#d4a017';
				c.fillRect(775, 285, 8, 8);
				// 沙发（装饰）
				c.fillStyle = '#3a4a7a';
				c.fillRect(40, 350, 130, 70);
				c.fillStyle = '#2b3559';
				c.fillRect(40, 350, 130, 18);
				c.fillStyle = '#3a4a7a';
				c.fillRect(34, 360, 16, 60);
				c.fillRect(160, 360, 16, 60);
				// 绿植
				c.fillStyle = '#2e7d32';
				c.fillRect(640, 440, 12, 40);
				c.beginPath();
				c.arc(646, 420, 26, 0, Math.PI * 2);
				c.fill();
				c.restore();
			}
			...
			drawHomeItems(c, t) {
				const HOME_TILE_MAP = {
					bag: { key: 'Inside_B', sx: 0, sy: 7 * 48 },          // Table A
					dex: { key: 'Inside_B', sx: 13 * 48, sy: 9 * 48 },     // Bookshelf A
					gallery: { key: 'Inside_B', sx: 5 * 48, sy: 4 * 48 },  // Window A
					craft: { key: 'Inside_C', sx: 4 * 48, sy: 9 * 48 },    // Blacksmith's Table
					shop: { key: 'Inside_B', sx: 0, sy: 6 * 48 },          // Sofa
				};
				c.save();
				...
				for (const item of this.homeItems) {
					...
					c.fillStyle = '#cfd3e4';
					c.fillText(item.label, item.x, item.y + bob + 42);
				}
				// F 交互提示
				if (this.homeNear) {
					...
					c.beginPath();
					c.arc(item.x, item.y - 44, 14, 0, Math.PI * 2);
					c.fill();
					c.strokeStyle = '#4f6ef7';
					c.lineWidth = 3;
					c.beginPath();
		...
		/** 合成台：科技合成动画——放入材料 → 压缩 → 拾取成品 */
		function CraftModal({ character, send, onClose }) {
			const inventory = Array.isArray(character?.inventory) ? character.inventory : [];
			const frags = inventory.filter((x) => x === 'skill-fragment').length;
			const [stage, setStage] = useState('idle'); // idle | compress | done
			const [progress, setProgress] = useState(0);
			useEffect(() => {
				if (stage !== 'compress') return;
				const start = Date.now();
				const dur = 2200;
				const timer = setInterval(() => {
					const p = Math.min(1, (Date.now() - start) / dur);
					setProgress(p);
					if (p >= 1) { clearInterval(timer); setStage('done'); }
				}, 50);
				return () => clearInterval(timer);
			}, [stage]);
			const craft = () => {
				if (frags < 10 || stage !== 'idle') return;
				setProgress(0);
				setStage('compress');
			};
			const pickup = () => {
				send({ kind: ClientMsg.CRAFT_ITEM, item: 'skill-book' });
				onClose();
			};
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 560, minHeight: 380, background: '#0e1017', border: '2px solid #4f6ef7', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14 }, children: [
					h('h2', { key: 'h', children: '⚒️ 科技合成台' }),
					h('div', { key: 'sub', className: 'sub', children: '把技能书碎片放入合成区，科技会压缩出完整技能书。当前：' + frags + ' / 10' }),
					hs('div', { key: 'slots', className: 'dsh-vs-fliprow', children: Array.from({ length: 10 }, (_, i) => {
						const filled = i < frags;
						return h('div', { key: i, style: { width: 44, height: 44, background: filled ? '#221a3a' : '#0a0c12', border: '1px solid ' + (filled ? '#7c5cfc' : '#2a2e3d'), borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }, children: filled ? h('img', { key: 'i', src: '/vs-game/assets/items/mv/mat-scroll.png', style: { width: 32, height: 32, imageRendering: 'pixelated' } }) : null });
					}) }),
					hs('div', { key: 'anim', style: { width: 340, height: 150, background: '#1c1f2b', border: '1px solid #2a2e3d', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', overflow: 'hidden' }, children: [
						stage === 'done'
							? h('img', { key: 'book', src: '/vs-game/assets/items/skill_book.png', style: { width: 72, height: 72, imageRendering: 'pixelated' } })
							: stage === 'compress'
								? h('div', { key: 'loading', style: { color: '#40c4ff', fontSize: 14, fontWeight: 700 }, children: '⚡ 科技压缩中 ' + Math.round(progress * 100) + '%' })
								: hs('div', { key: 'drop', style: { display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }, children: [
									h('div', { key: 'i', style: { fontSize: 42 }, children: '📥' }),
									h('div', { key: 't', className: 'sub', children: '请把 10 个碎片放入合成区' }),
								] }),
					] }),
					stage === 'idle'
						? h('button', { key: 'bt', className: 'dsh-vs-btn' + (frags < 10 ? ' ghost' : ''), disabled: frags < 10, onClick: craft, children: '放入合成' })
						: stage === 'compress'
							? h('div', { key: 'wait', className: 'sub', children: '请稍候……' })
							: h('button', { key: 'pick', className: 'dsh-vs-btn', onClick: pickup, children: '🎒 拾取成品' }),
					h('button', { key: 'c', className: 'dsh-vs-btn ghost', onClick: onClose, children: '关闭' }),
				] }),
			] });
		}
		...
		function ShopModal({ character, send, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 620, minHeight: 460, backgroundImage: 'url("/vs-game/assets/ui/shop.png")', backgroundSize: '100% 100%', borderRadius: 18, padding: 34, display: 'flex', flexDirection: 'column', gap: 12, color: '#e6e8f0' }, children: [
					h('h2', { key: 'h', children: '🛒 网上商店' }),
					h('div', { key: 'g', className: 'sub', children: '💰 金币：' + gold + ' · 点击物品后右侧设置数量并确认交易。' }),
					hs('div', { key: 'tabs', style: { display: 'flex', gap: 8 }, children: [
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderPanel = () => {
				...
				const attack = BASE_ATTACK * (1 + 0.20 * mightLv);
				...
				return hs('div', { className: 'dsh-vs-char-panel', children: [
					...
					h('div', { key: 'atk', className: 'dsh-vs-upgrade-row', children: [
						...
						h('span', { key: 'v', className: 'dsh-vs-upgrade-cost', children: attack.toFixed(1) }),
					] }),
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				style: { transform: 'translate(calc(-50% + ' + offset.x + 'px), calc(-50% + ' + offset.y + 'px)) scale(' + winScale + ')' },
				children: [
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'bossintro' && s.bossIntro ? hs('div', {
						...
						children: [
							...
							h('div', { key: 'hm', className: 'dsh-vs-dialog-hint', children: '点击继续 ▶' }),
						] })
					: null,
				...
				galleryOpen ? h(GalleryModal, { key: 'gallery', levels, character, onClose: () => setGalleryOpen(false) }) : null,
				shopOpen ? h(ShopModal, { key: 'shop', character, send, onClose: () => setShopOpen(false) }) : null,
