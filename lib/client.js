...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// 游戏窗口（居中）
			'.dsh-vs-win{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);',
		...
		class GameEngine {
			...
			tick(dt) {
				...
				// 工作活跃（10s 内收到过燃料）时待机刷怪让位；chaos 期间加倍
				if (this.elapsed - this.lastFuelElapsed > 10) this.idleSpawn(dt);
				...
				if (p.invuln <= 0) {
					for (const e of this.grid.query(p.x, p.y, 40)) {
						if (Math.hypot(e.x - p.x, e.y - p.y) < e.size * 0.55 + 13) {
							const raw = 8 + Math.min(20, this.elapsed / 30) + (e.elite ? 4 : 0);
				...
				if (p.invuln <= 0) {
					for (const b of this.enemyBullets) {
						if (Math.hypot(b.x - p.x, b.y - p.y) < 15) {
							b.life = 0;
							const dmg = Math.max(1, 6 + Math.min(12, this.elapsed / 40) - p.passives.armor);
							p.hp -= dmg;
							p.invuln = 0.5;
		...
		function Hud({ snap }) {
			return hs('div', { className: 'dsh-vs-hud', children: [
				...
				h('div', { key: 'tr', className: 'dsh-vs-hud-tr', children: [
					...
					snap.buffs.length > 0 ? h('div', { key: 'b', style: { color: '#ffd54f' }, children: snap.buffs.map((b) => b.icon + b.left + 's').join('  ') }) : null,
				] }),
		...
		function GameWindow({ onClose, wsStatus, send }) {
			...
			const s = snap;
			return hs('div', { className: 'dsh-vs-win', children: [
				h('div', { key: 'head', className: 'dsh-vs-head', children: [
