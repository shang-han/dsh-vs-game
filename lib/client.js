		...
		class GameEngine {
			...
			reset() {
				...
				this.player = {
					...
					invuln: 0, celebrate: 0, facing: 1, aimAngle: 0, moving: false,
			...
			fireMine(lv) {
				// 阔剑地雷：布设时朝向最近威胁，进行定向扇形爆破（强化：触发范围/扇形距离/伤害提升）
				// 进化·垃圾回收：存 6 雷、伤害翻倍、大扇形全减速
				const evo = this.isEvolved('mine');
				const cap = evo ? 6 : lv >= 2 ? 4 : 3;
				if (this.mines.length >= cap) this.mines.shift();
				const p = this.player;
				const target = this.findBoltTarget(p.x, p.y, false, 280);
				this.mines.push({
					x: p.x, y: p.y,
					angle: target ? Math.atan2(target.y - p.y, target.x - p.x) : (p.aimAngle ?? (p.facing >= 0 ? 0 : Math.PI)),
			...
			updateMines(dt) {
				for (const m of this.mines) {
					...
					const near = this.grid.query(m.x, m.y, m.trigger + 20).some((e) => Math.hypot(e.x - m.x, e.y - m.y) < m.trigger);
					if (near) {
						m.dead = true;
			...
			drawMines(c, t) {
				for (const m of this.mines) {
					...
					c.rotate(m.angle);
					// 阔剑扇形危险区
					c.fillStyle = m.arm > 0 ? 'rgba(255,183,77,0.10)' : 'rgba(255,183,77,0.20)';
					c.beginPath();
					c.moveTo(0, 0);
					c.arc(0, 0, m.range, -halfArc, halfArc);
					c.closePath();
					c.fill();
					c.strokeStyle = 'rgba(255,183,77,0.35)';
					c.lineWidth = 1;
					c.beginPath();
					c.moveTo(0, 0);
					c.arc(0, 0, m.range, -halfArc, halfArc);
