...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		function ClearPanel({ snap, character, story, send, onExit, onRetry }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				...
				story && Array.isArray(story.post) && story.post.length > 0 ? hs('div', { key: 'post', style: { maxWidth: 620, display: 'flex', flexDirection: 'column', gap: 4, margin: '2px 0' }, children: story.post.map((line, i) => hs('div', { key: i, style: { fontSize: 13, lineHeight: 1.6, textAlign: 'center' }, children: [
					h('b', { key: 'sp', style: { color: '#9d6bff' }, children: (line.speaker ?? '') + '：' }),
					h('span', { key: 'tx', children: line.text ?? '' }),
				] })) }) : null,
				story && story.artPost && !(Array.isArray(story.post) && story.post.length > 0) ? h('img', { key: 'art', src: '/vs-game/' + story.artPost, alt: '通关', style: { width: 190, borderRadius: 12, boxShadow: '0 6px 24px rgba(0,0,0,.5)' } }) : null,
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'home' && s.activeSkill ? h('button', {
						key: 'homeSkill',
						className: 'dsh-vs-skill-btn has-e dsh-vs-skill-btn-home' + (s.activeSkill.type === 'strike' && s.activeSkill.timer > 0 ? ' on' : ''),
						onClick: () => engineRef.current?.activateActiveSkill(),
						children: (() => {
							const sk = s.activeSkill;
							const suffix = sk.type === 'strike'
								? (sk.timer > 0 ? ' ' + Math.ceil(sk.timer) + 's' : (sk.cd > 0 ? ' CD ' + Math.ceil(sk.cd) + 's' : '（E）'))
								: (sk.enabled ? ' 已开启' : ' 已关闭');
							return sk.iconUrl
								? [h('img', { key: 'i', src: sk.iconUrl, alt: '', style: { width: 16, height: 16, verticalAlign: 'middle', imageRendering: 'pixelated' } }), ' ' + sk.name + suffix]
					...
					s && s.phase === 'bossintro' && s.bossIntro ? hs('div', {
						...
						children: [
							s.bossIntro.kind === 'post' && activeLevel?.story?.artPost ? h('img', { key: 'postArt', src: '/vs-game/' + activeLevel.story.artPost, alt: '', style: { position: 'absolute', left: '50%', bottom: 145, transform: 'translateX(-50%)', width: 220, borderRadius: 12, boxShadow: '0 8px 28px rgba(0,0,0,.6)', pointerEvents: 'none' } }) : null,
