...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function CraftModal({ character, onStart, onClose }) {
			...
			const add = (item) => {
				...
				if (cur < total) setPlaced([...placed, item]);
			};
			const remove = (item) => {
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, onStart: () => { engineRef.current?.startCraftRitual(); setCraftOpen(false); }, onClose: () => { setCraftOpen(false); engineRef.current?.focusCanvas(); } }) : null,
