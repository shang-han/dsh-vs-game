...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-upd{display:flex;align-items:center;gap:8px;flex-wrap:wrap;}',
			'.dsh-vs-upd-msg{color:#8a8fa3;font-size:11px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}',
			'.dsh-vs-upd-out{margin:0;width:100%;max-height:120px;overflow:auto;background:#0a0c12;border:1px solid #1d2130;border-radius:6px;padding:6px;color:#9aa2b1;font-size:10px;white-space:pre-wrap;}',
			// 选关面板
			...
			spawnEnemy(type, count, elite) {
				for (let i = 0; i < count; i++) {
					...
					this.enemies.push({
						...
						color: base.color, label: base.label,
						xp: base.xp * (elite ? 3 : 1),
						elite: !!elite, hitFlash: 0, slow: 0, aggro: !this.level,
						wanderA: rand(0, Math.PI * 2), wanderT: rand(0.8, 2.2),
					});
				}
			}
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			const checkUpdate = async () => {
				setUpdateInfo(null);
				try {
					const r = await fetch('/vs-game/update');
			...
			const pullUpdate = async () => {
				...
				setUpdating(true);
				try {
					const r = await fetch('/vs-game/update', { method: 'POST' });
					...
					setUpdateInfo({ ...d, output: '' });
					const poll = async () => {
						try {
							const jr = await fetch('/vs-game/update-job?id=' + d.jobId);
							const jd = await jr.json();
							if (!jd.ok) throw new Error(jd.error || 'poll failed');
							setUpdateInfo({ ...jd, ok: true });
							...
							setUpdateInfo({ ok: false, error: String(e?.message ?? e), output: '' });
							setUpdating(false);
						}
					};
					poll();
			...
			return hs('div', {
				...
				cfgOpen && cfg ? hs('div', { key: 'pop', className: 'dsh-vs-pop', children: [
					...
					h('div', { key: 'upd', className: 'dsh-vs-upd', children: [
						h('button', { key: 'chk', className: 'dsh-vs-mini-btn', onClick: checkUpdate, children: '检查更新' }),
						updateInfo ? h('span', { key: 'msg', className: 'dsh-vs-upd-msg', children: updateInfo.ok
							? (updateInfo.status === 'done'
								? '更新完成'
								: updateInfo.status === 'failed'
									? '更新失败'
									: updateInfo.isLatest
										? '已是最新 ' + (updateInfo.latest ? String(updateInfo.latest).slice(0, 8) : '')
										: '最新 ' + (updateInfo.latest ? String(updateInfo.latest).slice(0, 8) : '未知'))
							: '检查失败：' + (updateInfo.error || '') }) : null,
						updateInfo?.ok && updateInfo.latest && !updateInfo.isLatest && updateInfo.status !== 'done' && updateInfo.status !== 'failed' ? h('button', { key: 'pull', className: 'dsh-vs-mini-btn', disabled: updating, onClick: pullUpdate, children: updating ? '更新中…' : '拉取更新' }) : null,
						updateInfo?.output ? h('pre', { key: 'out', className: 'dsh-vs-upd-out', children: String(updateInfo.output).slice(-1200) }) : null,
					] }),
				] }) : null,
