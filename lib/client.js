		...
		class GameEngine {
			...
			drawHomeObjects(c, t) {
				...
				for (let i = 0; i < (this.homeChests ?? []).length; i++) {
					...
					const pos = this.chestHomePos(i);
					const kind = this.homeChests[i]?.kind;
					const img = assetImg(kind === 'record-player' ? '/vs-game/assets/items/mv/record-player.png' : kind === 'diamond-chest' ? '/vs-game/assets/items/mv/chest-blue.png' : '/vs-game/assets/items/mv/wooden-chest.png');
		...
		function ChestModal({ character, index, send, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 660, minHeight: 420, background: '#0e1017', border: '2px solid #8a6b3d', borderRadius: 18, padding: 24, display: 'flex', flexDirection: 'column', gap: 12 }, children: [
					h('h2', { key: 'h', style: { display: 'flex', alignItems: 'center', gap: 8 }, children: kind === 'diamond-chest' ? [h('img', { key: 'i', src: '/vs-game/assets/items/mv/chest-blue.png', alt: '', style: { width: 32, height: 32, imageRendering: 'pixelated' } }), '钻石宝箱'] : '📦 木制宝箱' }),
					hs('div', { key: 'body', style: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }, children: [
						hs('div', { key: 'left', style: { display: 'flex', flexDirection: 'column', gap: 6 }, children: [
			...
			return hs('div', {
				...
				chestMenu != null ? (() => {
					...
					return h(ChestActionModal, { key: 'chestmenu',
						title: cm?.kind === 'record-player' ? '🎵 唱片机' : cm?.kind === 'diamond-chest' ? h('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 6 } }, [h('img', { key: 'i', src: '/vs-game/assets/items/mv/chest-blue.png', alt: '', style: { width: 22, height: 22, imageRendering: 'pixelated' } }), '钻石宝箱']) : '📦 木制宝箱',
