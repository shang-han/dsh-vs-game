		...
		class GameEngine {
			...
			drawHome(c, t) {
				...
				if (furniture.complete && furniture.naturalWidth) {
					// Chest of Drawers（index 139）
					c.drawImage(furniture, 11 * 48, 8 * 48, 48, 48, 60, 150, 56, 72);
					// Large Bed：竖着放（第6行第9格 + 第7行第9格），把横版两格旋转90°
					c.save();
					c.translate(408, 288); // 两格中心
					c.rotate(-Math.PI / 2);
					c.drawImage(furniture, 9 * 48, 10 * 48, 48, 48, -48, -24, 48, 48);
					c.drawImage(furniture, 10 * 48, 10 * 48, 48, 48, 0, -24, 48, 48);
					c.restore();
				} else {
					c.fillStyle = '#4a2e1d';
					c.fillRect(60, 150, 56, 72);
					c.fillRect(110, 280, 96, 96);
				}
				c.restore();
			}
