...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			'.dsh-vs-pop-scale{color:#9d6bff;font-weight:700;min-width:36px;text-align:right;}',
			'.dsh-vs-pop input[type=range]{flex:1;min-width:0;accent-color:#7c5cfc;}',
			// 选关面板
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				style: { transform: 'translate(calc(-50% + ' + offset.x + 'px), calc(-50% + ' + offset.y + 'px)) scale(' + (cfg?.windowScale ?? 1) + ')' },
				children: [
				h('div', {
				...
				cfgOpen && cfg ? hs('div', { key: 'pop', className: 'dsh-vs-pop', children: [
					...
					h('label', { key: 'ws', children: [
						h('span', { key: 't', children: '窗口缩放' }),
						h('span', { key: 'v', className: 'dsh-vs-pop-scale', children: (cfg.windowScale ?? 1).toFixed(2) + 'x' }),
						h('input', { key: 'i', type: 'range', min: 0.5, max: 2, step: 0.05, value: cfg.windowScale ?? 1, onChange: (e) => patchCfg({ windowScale: Number(e.target.value) }) }),
					] }),
				] }) : null,
