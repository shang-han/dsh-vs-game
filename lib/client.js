	...
	factory: (require) => {
		...
		class GameEngine {
			...
			buildHomeItems() {
				...
				if (this.clearedLevels?.has('busy-server')) {
					items.push({ id: 'craft', x: 340, y: 430, icon: '⚒️', label: '合成台' });
				}
		...
		/** 网上商店：使用 shop.png 边框背景，目前只支持卖出 */
		function ShopModal({ character, send, onClose }) {
			const inventory = Array.isArray(character?.inventory) ? character.inventory : [];
			const counts = new Map();
			for (const it of inventory) counts.set(it, (counts.get(it) ?? 0) + 1);
			const rows = [...counts.entries()].filter(([item]) => !['newbie-gift', 'skill-book'].includes(item));
			const price = (item) => item.startsWith('acc-') ? 150 : item.startsWith('mat-') ? 80 : item === 'skill-fragment' ? 50 : item.startsWith('tool-') ? 120 : 10;
			return hs('div', { className: 'dsh-vs-cover', children: [
				hs('div', { key: 'panel', style: { width: 580, minHeight: 420, backgroundImage: 'url("/vs-game/assets/ui/shop.png")', backgroundSize: '100% 100%', borderRadius: 18, padding: 38, display: 'flex', flexDirection: 'column', gap: 12, color: '#e6e8f0' }, children: [
					h('h2', { key: 'h', children: '🛒 网上商店' }),
					h('div', { key: 'g', className: 'sub', children: '💰 金币：' + (Number(character?.gold) || 0) + ' · 目前只开放卖出，之后会追加购买。' }),
					hs('div', { key: 'list', className: 'dsh-vs-stats', children: rows.length === 0
						? [h('div', { key: 'empty', children: '没有可卖出的物品' })]
						: rows.map(([item, count]) => {
							const meta = itemMeta(item);
							return hs('div', { key: item, style: { display: 'flex', alignItems: 'center', gap: 10, background: 'rgba(10,12,18,.65)', border: '1px solid #2a2e3d', borderRadius: 8, padding: '8px 12px' }, children: [
								h('span', { key: 'i', children: meta.iconUrl ? h('img', { key: 'ii', src: meta.iconUrl, style: { width: 24, height: 24, imageRendering: 'pixelated' } }) : meta.icon }),
								h('span', { key: 'n', children: meta.name + (count > 1 ? ' ×' + count : '') }),
								h('button', { key: 's', className: 'dsh-vs-mini-btn', onClick: () => send({ kind: ClientMsg.SELL_ITEM, item }), children: '+ ' + price(item) + ' 金币 · 卖出' }),
							] });
						}) }),
		...
		function GameWindow({ onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, send, onClose: () => setCraftOpen(false) }) : null,
