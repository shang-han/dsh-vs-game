	...
	factory: (require) => {
		...
		class GameEngine {
			...
			accessoryAttack() {
				...
				for (const it of this.accessories ?? []) {
					...
					if (meta?.attack) sum += (Number(meta.attack[0]) + Number(meta.attack[1])) / 2;
				}
				return sum;
			}
			attackPower() { return BASE_ATTACK * this.dmgMul() + this.accessoryAttack(); }
		...
		function CharacterModal({ character, goldEarned, send, onClose }) {
			...
			const renderBag = () => {
				...
				for (let i = 0; i < slotCount; i++) {
					...
					nodes.push(h('div', {
						key: item,
						className: 'dsh-vs-item' + ((canOpen || canEquip) ? ' use' : '') + (meta.quality ? ' ' + meta.quality : '') + (isSel ? ' selected' : ''),
			...
			const renderPanel = () => {
				...
				const accAtk = accessories.reduce((sum, it) => {
					const m = it ? itemMeta(it) : null;
					return sum + (m?.attack ? (Number(m.attack[0]) + Number(m.attack[1])) / 2 : 0);
				}, 0);
				const attack = BASE_ATTACK * (1 + 0.20 * mightLv) + accAtk;
		...
		function GameWindow({ standalone, onClose, wsStatus, send, helloRef, character, goldEarned, levels }) {
			...
			return hs('div', {
				...
				craftOpen ? h(CraftModal, { key: 'craft', character, placed: craftPlaced, onChange: (items) => { setCraftPlaced(items); engineRef.current?.setCraftPlaced(items); }, onStart: () => { engineRef.current?.startCraftRitual(); setCraftOpen(false); engineRef.current?.focusCanvas(); }, onClose: () => { setCraftOpen(false); engineRef.current?.focusCanvas(); } }) : null,
