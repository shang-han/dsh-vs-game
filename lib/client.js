...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			attachInput() {
				...
				this._onUiClick = (e) => {
					const t = e.target;
					if (t && t.closest && t.closest('button, .dsh-vs-card, .dsh-vs-item, .dsh-vs-chip')) playSE('ui-click', 0.45, 0.03);
				};
			...
			tryDash(tx, ty) {
				...
				this.player.skillAction = { t: 0, dur: 0.22, dx: (tx - x1) / dLen, dy: (ty - y1) / dLen };
				this.player.actionCd = 0.22;
			...
			updatePlayer(dt) {
				...
				if (p.moving) {
					...
					if (dx !== 0) p.facing = dx > 0 ? 1 : -1;
					// 划除激活期间：移动时做一次刺击动作
					if (this.skillTimer > 0 && p.actionCd <= 0) {
						p.skillAction = { t: 0, dur: 0.18, dx: dx / len, dy: dy / len };
						p.actionCd = 0.26;
					}
				}
				p.x = Math.max(16, Math.min(this.world.w - 16, p.x));
			...
			drawPlayer(c, t) {
				...
				const act = p.skillAction;
				const prog = act ? Math.min(1, act.t / act.dur) : 0;
				const punch = act ? Math.sin(prog * Math.PI) : 0;
				const bx = p.x + (act ? act.dx * punch * 8 : 0);
				const by = p.y + (act ? act.dy * punch * 8 : 0);
				const drawn = drawSprite(c, state, t, bx, by, 56, flip);
				if (!drawn) {
					c.fillStyle = '#7c5cfc';
					c.beginPath();
				...
				if (this.skillTimer > 0) {
					const aim = act ? Math.atan2(act.dy, act.dx) : (p.facing > 0 ? 0 : Math.PI);
					const handX = bx + (flip ? -11 : 11);
					const handY = by + 3;
					c.save();
					...
					c.shadowBlur = act ? 8 : 3;
					c.strokeStyle = '#f6f3ea';
					c.lineWidth = 5;
					c.beginPath();
					c.moveTo(0, 0);
					c.lineTo(14 + punch * 12, 0);
					c.stroke();
					c.restore();
				}
				c.globalAlpha = 1;
