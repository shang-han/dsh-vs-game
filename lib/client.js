...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			enderBurst(x, y) {
				this.burst(x, y, '#b46bff', 30);
				this.particles.push({ kind: 'enderPillar', x, y, life: 0.38, maxLife: 0.38 });
				for (let i = 0; i < 10; i++) {
					this.particles.push({ kind: 'ender', x, y, life: 0.4 + i * 0.035, maxLife: 0.75, color: '#b46bff', size: 24 + i * 7 });
				}
			}
			...
			drawParticles(c) {
				for (const pt of this.particles) {
					...
					if (pt.kind === 'spark') {
						...
						const grad = c.createLinearGradient(pt.x, pt.y - 52, pt.x, pt.y + 30);
						grad.addColorStop(0, 'rgba(180,107,255,0)');
						grad.addColorStop(0.45, 'rgba(180,107,255,' + (0.55 * k2) + ')');
						grad.addColorStop(1, 'rgba(80,40,160,0)');
						c.fillStyle = grad;
						c.fillRect(pt.x - 26, pt.y - 52, 52, 82);
						c.strokeStyle = 'rgba(216,180,255,' + (0.8 * k2) + ')';
						c.lineWidth = 2;
						c.beginPath(); c.ellipse(pt.x, pt.y + 18, 22 * (1.1 - k2 * 0.2), 7, 0, 0, Math.PI * 2); c.stroke();
