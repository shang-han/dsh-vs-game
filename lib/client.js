...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ITEM_DEFS = {
			...
			'skill-fragment-teleport': { icon: '🟣', iconUrl: '/vs-game/assets/items/ender_pearl.png', name: 'TP碎片', type: 'material', desc: '第二关 Boss 掉落，收集 10 个合成 TP 技能书' },
			...
			'skill-book-teleport': { icon: '🟣', iconUrl: '/vs-game/assets/items/ender_pearl.png', name: '技能书·TP', type: 'consumable', desc: '使用后永久解锁主动技能「TP」' },
...
const BAG_STORAGE_KEY = 'dsh-vs-game-bag-slots-v1';
		...
		class GameEngine {
			...
			attachInput() {
				...
				this._onPointerDown = (e) => {
					...
					if (this.phase === 'home') {
						...
						// 激光技能：点击屏幕发射，家里也能用
						if (this.isLaserSkill()) { this.fireLaserSkill(x, y); return; }
						// 人物界面交互：点家具打开，点地面移动
			...
			activateActiveSkill() {
				...
				if (sk?.type) { this.setBanner('🌀 ' + sk.name + '：点击屏幕发射'); return; }
				if (this.skillTimer > 0 || !sk || this.skillCd > 0) return;
				this.skillCd = sk.cd;
			...
			tryDash(tx, ty) {
				...
				this.player.skillAction = { t: startT, dur: 0.55, dx: (tx - x1) / dLen, dy: (ty - y1) / dLen };
			...
			fireLaserSkill(tx, ty) {
				const sk = ACTIVE_SKILLS[this.activeSkillId];
				if (!sk || !sk.type || this.laserSkillCd > 0) return false;
				...
				this.burst(this.player.x, this.player.y, sk.type === 'teleport' ? '#b46bff' : '#40c4ff', 8);
				return true;
			}
			...
			updateSkillLasers(dt) {
				...
				for (const l of this.skillLasers) {
					...
					if (!l.done && l.t >= l.dur) {
						l.done = true;
						if (l.type === 'teleport') {
							this.burst(l.x1, l.y1, '#b46bff', 18);
							this.burst(l.x2, l.y2, '#7b2fc0', 26);
							this.player.x = Math.max(16, Math.min(this.world.w - 16, l.x2));
							this.player.y = Math.max(16, Math.min(this.world.h - 16, l.y2));
							this.updateCamera();
							playSE('lightning', 0.65, 0.08);
						} else {
							this.burst(l.x2, l.y2, '#40c4ff', 18);
			...
			drawSkillLasers(c) {
				...
				for (const l of this.skillLasers) {
					...
					c.save();
					c.lineCap = 'round';
					c.shadowColor = isTp ? 'rgba(180,107,255,.9)' : 'rgba(64,196,255,.9)';
					c.shadowBlur = 16;
					c.strokeStyle = isTp ? '#b46bff' : '#40c4ff';
					c.lineWidth = isTp ? 6 : 5;
					c.beginPath();
					c.moveTo(l.x1, l.y1);
					c.lineTo(x, y);
					c.stroke();
			...
			// ── 主更新 ──
			tick(dt) {
				if (this.phase === 'home') { this.updateHome(dt); this.updateSkillLasers(dt); return; }
			...
			drawPlayer(c, t) {
				...
				if (this.phase === 'home') state = p.moving ? 'walk' : 'idle';
				else if (this.phase === 'gameover') state = 'disappointed';
				else if (this.skillTimer > 0 && p.skillAction) state = 'strike';
				...
				const faceRight = state === 'strike' && p.skillAction ? p.skillAction.dx > 0 : p.facing > 0;
				let flip = faceRight;
				// 黑白房 + Billie Jean 彩蛋：移动方向不变，但左右动作镜像，看起来像倒着走
				if (this.easterEgg && this.homeRoom === 1) flip = !flip;
				let spriteT = t;
				if (state === 'strike' && p.skillAction) {
			...
			return hs('div', {
				...
				hs('div', { key: 'stage', className: 'dsh-vs-stage', children: [
					...
					s && s.phase === 'playing' && s.activeSkill ? h('button', {
						key: 'skill',
						className: 'dsh-vs-skill-btn' + (s.activeSkill.type === 'strike' && s.activeSkill.timer > 0 ? ' on' : ''),
						onClick: () => engineRef.current?.activateActiveSkill(),
						children: s.activeSkill.type !== 'strike'
							? (s.activeSkill.icon + ' ' + s.activeSkill.name + (s.activeSkill.internalCd > 0 ? ' ' + s.activeSkill.internalCd.toFixed(1) + 's' : '（点击屏幕发射）'))
