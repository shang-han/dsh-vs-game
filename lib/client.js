...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ITEM_META = {
			...
			'skill-fragment': { icon: '📕', name: '技能书碎片·划除', desc: '收集 10 个可在合成台合成技能书' },
		};
		...
		class GameEngine {
			...
			drawHomeItems(c, t) {
				...
				for (const item of this.homeItems) {
					...
					c.beginPath();
					c.arc(item.x, item.y + bob, 24, 0, Math.PI * 2);
					c.fill();
		...
		function CraftModal({ character, send, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				h('h2', { key: 'h', children: '⚒️ 合成台' }),
				h('div', { key: 'sub', className: 'sub', children: '把技能书碎片合成完整的技能书。当前：' + frags + ' / 10' }),
				h('button', {
					key: 'craft',
					className: 'dsh-vs-btn' + (frags < 10 ? ' ghost' : ''),
					disabled: frags < 10,
					onClick: () => send({ kind: ClientMsg.CRAFT_ITEM, item: 'skill-book' }),
					children: '合成「技能书·划除」',
				}),
		...
		function ShopModal({ character, send, onClose }) {
			...
			return hs('div', { className: 'dsh-vs-cover', children: [
				...
				h('div', { key: 'sub', className: 'sub', children: '目前只开放卖出，之后会追加购买。' }),
				hs('div', { key: 'list', className: 'dsh-vs-stats', children: rows.length === 0
					? [h('div', { key: 'empty', children: '没有可卖出的物品' })]
					: rows.map(([item, count]) => {
						const meta = itemMeta(item);
						return hs('div', { key: item, style: { display: 'flex', alignItems: 'center', gap: 10, background: '#0e1017', border: '1px solid #2a2e3d', borderRadius: 8, padding: '8px 12px' }, children: [
							h('span', { key: 'i', children: meta.icon }),
